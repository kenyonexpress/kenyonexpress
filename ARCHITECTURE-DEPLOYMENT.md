...
### 4.2 ⛔ התנגשות מספרי מיגרציה בין ענפים

**חוסם מיזוג.** שני ענפים מגדירים את אותם מספרים על קבצים שונים:
...
מיזוג של השניים ייצור שני קבצים לכל מספר, וסדר ההחלה יהפוך תלוי בשם הקובץ
ולא בכוונה. יש למספר מחדש אחד הזוגות **לפני** המיזוג, ולוודא שאף אחד מהם לא
הוחל פעמיים תחת שני שמות.

מצב ההחלה בפועל: ‏`090_profiles_no_self_role_change` **הוחלה** לפרודקשן
ב-2026-07-28, כי היא סוגרת הסלמת הרשאות פעילה. שלושת האחרים לא הוחלו.
