...
נימוק ל-07:00: זו תחילת יום העסקים, לפני שהבעלים פותח את הדשבורד; וכל שרשרת
ה-jobs הלילית (expire 01:50 -> rollup 02:10 -> matviews 02:40 -> partitions 03:00)
כבר הסתיימה שעות קודם, כך שהמייל מדווח יום-אתמול סופי וסגור, לא נתון חלקי.
נמען יחיד: כתובת האדמין מ-env (`ADMIN_DIGEST_EMAIL`); ריבוי נמענים נדחה עד
שיש יותר מבעלים אחד. מייל תפעולי, לא שיווקי: בלי gate הסכמה ובלי שעות שקט.
...
נימוק: אין פרסום בתשלום, ולכן אין צרכן לנתוני click-ID או לסיווג referrer;
הערוצים היחידים שקיימים היום (שיתופי וואטסאפ, קמפיינים אורגניים, ניוזלטר)
נשלטים במלואם על ידינו ומתויגים ב-`utm_*` בקישור עצמו. כל שכבה מעבר לזה
מוסיפה קוד, משטח פרטיות ותחזוקה בלי אף החלטה שהיא משרתת עכשיו, והדחייה
לא עולה כלום: מבנה ה-jsonb פתוח וההרחבה עתידית היא שינוי SDK בלבד.
...
| רכיב | קובץ |
|---|---|
| דלתת v3 כמיגרציה חדשה | `supabase/migrations/053_analytics_v3.sql` |
