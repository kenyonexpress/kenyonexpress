...
הגלים רצים בסדר שהם מופיעים בקובץ הזה: תור סגור (1-9), אחריו DB HARDENING (10-15), אחריו COUPON STOREFRONT (16-21), אחריו MONEY-WHOLE-SHEKELS (22-25), ואחריו (26).
המילה AUTOPILOT-DONE בשורה הראשונה של STATE.md היא תנאי העצירה של autopilot.sh. אל תכתוב אותה בסוף גל. רק ב-(26), כשכל הגלים סגורים.
...
[גל MONEY-WHOLE-SHEKELS - נמסר על ידי אופיר 19.08]
(22) החלטת כסף רשמית ומחייבת: מחירים בשקלים שלמים בלבד, בלי אגורות. שים לב לשלוש עובדות שנמדדו ב-19.08 לפני שאתה נוגע בקוד, אל תתקן אותן לפי הניסוח בבקשה:
     א. `packages/money.ts` לא קיים. אין תיקיית `packages/` בריפו כלל. הקבצים הם `src/lib/money.ts` ו-`src/lib/commerce/money.ts`.
     ב. `docs/BUSINESS-MODEL-RULES.md` לא קיים. קיים `docs/BUSINESS-MODEL.md`, אבל `docs/` שייכת לסוכן Cursor והיא תנאי עצירה בראש הקובץ הזה. לכן אל תכתוב ל-`docs/`. ההחלטה המחייבת נכתבת ב-`CLAUDE.md` וב-`STATE.md` בלבד, ומ-`STATE.md` מפנים ל-`docs/BUSINESS-MODEL.md` כקריאה בלבד.
     ג. "בלי אגורות" אינו ביטול כלל האגורות. `CLAUDE.md:66` קובע שכסף נשמר כ-integer באגורות, וזו יחידת האחסון. "שקלים שלמים" הוא אילוץ על הערך, לא על היחידה. לכן האכיפה היא `amount_agorot % 100 = 0`, ולא מעבר לאחסון בשקלים. מעבר לאחסון בשקלים היה סותר את `CLAUDE.md:66` ואת `src/lib/commerce/money.ts` ומכפיל כל מחיר קיים ב-DB פי 100 או מחלק בו.
     האכיפה בכל שכבה, על עמודות המחיר ללקוח בלבד ולא על עמודות שער או אחוז: CHECK constraint `% 100 = 0` במיגרציה חדשה תחת `migrations/pending/`, סכימת Drizzle, ולידציית Zod, טפסי אדמין (`step=100` באגורות, בלי נקודה עשרונית בקלט), ותצוגה בלי אגורות. אל תחיל את המיגרציה על פרודקשן בלי אישור נפרד, זה תנאי עצירה.
(23) `PENDING-109` בבקשה הוא `supabase/migrations/PENDING-109-recurring-subscriptions.sql`, והנושא שלו הוא סוג המוצר recurring והמנויים, לא עשרוניות במחיר. הקובץ היחיד שעוסק בעשרוניות הוא `PENDING-money-integer-fix.sql`, שממיר numeric(N,2) שקלים ל-integer אגורות, כלומר הוא מיישם את (22) ולא סותר אותו. לכן: אל תמחק אף אחד מהשניים בהרצה הזאת. מחיקת קבצים היא תנאי עצירה. במקום זה כתוב ב-`STATE.md` תחת "החלטות שהתקבלו לבד" איזה משני הקבצים אופיר התכוון להסיר, עם הציטוט מכל קובץ, והשאר את ההסרה לאישור מפורש.
(24) `PENDING-revoke_anon_writes.sql` כבר לא קיים ואין מה להריץ. הוא הוחל על פרודקשן ב-10.08 באישור אופיר דרך MCP ושמו שונה ל-`supabase/migrations/111_revoke_anon_writes.sql` (ראה שורה 4 בקובץ). האימות שהבקשה מזמינה כבר בוצע ומתועד שם: anon עם SELECT על 46 טבלאות ו-INSERT/UPDATE/DELETE על `carts` בלבד, ועגלת אורח נבדקה insert+update+delete בתוך DO block עם rollback. לכן במקום להחיל מחדש: (א) מדוד שוב את ה-grants של anon דרך MCP ואמת שהמצב לא נסוג, (ב) הרץ את זרימת עגלת האורח ו-checkout בטסטים ואמת שהיא עוברת, (ג) טפל בפער השיורי שהקובץ עצמו מתעד: `pg_default_acl` עבור `supabase_admin` עדיין נותן ל-anon `arwdDxtm`, כי MCP מריץ כ-`postgres` ותפקיד יכול לשנות רק את ברירות המחדל של עצמו. תעד ב-`STATE.md` אם אין דרך לסגור אותו דרך MCP.
(25) אחרי כל אחד מ-22 עד 24: pnpm type-check + pnpm lint + טסטים + commit + push + עדכון STATE.md עם "המשך מ: שלב X".

[סגירה סופית]
(26) רק כששלבים 10-25 כולם סגורים: כתוב את המילה AUTOPILOT-DONE בשורה הראשונה של STATE.md, צור tag, וכתוב דוח סיום ב-STATE.md.
