...
**‏diff פיקסלים** רץ גם הוא: ממוצע 33.7% על פני רצועות של 200px.
המספר הזה מודד **תוכן + פריסה**, כי `קופון-טסט` החי לא קיים ב-DB הזה —
שני הדפים מציגים מוצרים שונים. הטבלה המבנית למעלה היא אות הנאמנות האמיתי;
מספר הפיקסלים שימושי רק כמגמה בין ריצות.

### פערי facet שהתגלו תוך כדי הריצה
‏`/products?type=coupon` מסנן על עמודת `type` בלבד, בעוד שדף המוצר, העגלה
ומנוע העמלות מתייחסים גם ל-`is_coupon_enabled` כקופון. התוצאה:
‏`barbecue` (‏`type='physical'` + ‏`is_coupon_enabled=true`) מתומחר ומוצג
כקופון בדף המוצר, אבל **לא מופיע בפאסט הקופונים בקטלוג**.
‏`toProductDocument` ב-Meilisearch כבר עושה את זה נכון. **הקטלוג צריך ליישר קו.**
