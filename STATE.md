...
**התיקון האמיתי — להריץ את המיגרציה החסרה:**
```
Supabase > SQL Editor: הרץ את התוכן של
supabase/migrations/054_voucher_redemption.sql
```
היא אדיטיבית בלבד (`ADD COLUMN IF NOT EXISTS`), בלי אובדן נתונים.
...
## מה עדיין חסר להשקה
1. **להריץ את 054 בפרודקשן** (הסעיף למעלה). עד אז כל הקופונים מוצגים
   כלא-זמינים לרכישה.
