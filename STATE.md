...
| product | 10.71% | **16.59%** | פתוח, לא נבדק |

כלומר שלושה מארבעת הדפים במקומם, והפריט הפתוח היחיד הוא **דף המוצר**.

### מה שנמצא ולא תוקן

1. **דף המוצר עומד על 16.59% מול 10.71% שרשום.** לא נבדק, ורצפת הרעש שלו
   לא נמדדה. זו הבדיקה הבאה, ובאותו סדר: קודם לצלם את החי פעמיים.
2. **‏`CategoryProductCard` מגיש `<img>` גולמי**, כלומר `/category`,
