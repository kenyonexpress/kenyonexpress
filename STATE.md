...
Updated: 2026-08-03 (autonomous: [46] הרצפה של 238ms הייתה שאילתה אחת, ו-k6 L1 עבר)
...
## Last Completed
[46] **הרצפה של ~238ms לא הייתה PPR, היא הייתה שאילתה אחת לא מקאששת.**
‏TTFB הוא 4ms בשני הדפים; מה שעולה 268-289ms (מוצר) ו-273-327ms (קטגוריה) הוא
התשובה **המלאה**, מול סיבוב keep-alive חם ל-Supabase מכאן של **266-313ms**.
שני שורדים מ-[42]: ‏`RelatedProducts` קרא דרך `createClient()` רמה אחת מתחת
לארבע הטבלאות ש-[42] קאשש, ו-`generateMetadata` של הקטגוריה ירה שאילתה משלו.
אחרי: **1.8-6.9ms** ו-**6.5-14ms**, ודף המוצר **סטטי לגמרי** (`s-maxage=3600`
במקום `private, no-store`). **‏k6 L1 עבר, השער שאדום מאז [41]**: ‏product p95
‏**1120/884/751 ← 16.81ms** מול 800. שלושה דברים שהזירוז שבר ותוקנו: קליפת
‏RSC בניווט צד-לקוח, ‏effect של `pathname` שסגר את המיני-קארט, ו-8 מתוך 32
דילי הבית שמצביעים על מוצר שלא קיים. ‏**191/3/0** ב-Playwright.
...
## In Progress
אין. התור סגור. הפתוחים הם קונפיג/נתונים ב-Blocking Issues.
