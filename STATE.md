...
## 2026-07-29 — MEGA-5 שלב 1: כלי הגירה מוורדפרס

‏Branch: `feat/wp-migration`, ארבעה קומיטים. ‏743 טסטים, ‏tsc נקי.

### החלטות שהתקבלו אוטומטית

- **ה-branch יצא מ-`phase5/homepage` ולא מ-main.** היעד אמר "from main",
  אבל `origin/cursor/add-supabase-3c830` הוא 352 קומיטים מאחור ואין בו
  `supabase/migrations` בכלל, אין `scripts/wp-import`, אין
  `src/lib/storage/r2.ts` ואין `src/proxy.ts`. הוא אב קדמון טהור של
  phase5 (אפס קומיטים משלו), אז יציאה מ-phase5 לא מאבדת כלום ומונעת
  כתיבה מחדש של כל השלב ואז התנגשות במיזוג. זה אותו דפוס שתועד ב-
  `CONTRADICTIONS.md` ב-28.07, אז המרחק היה 161.
- **לא נבנה parser שני.** הצינור בן שישה השלבים כבר קיים, אז קוראי
  ה-XML/CSV פולטים את צורת השורה שה-transform כבר מקבל. אף שלב במורד
  הזרם לא נגע.
- **אין טבלאות `wp_product_map` / `wp_category_map`.** שתי העובדות כבר
  יושבות ב-`wp_import.id_map`, שדרכו כל שלב עושה upsert. טבלה שנייה
  הייתה מקור אמת שני שמתפצל בריצה חלקית ראשונה. השמות קיימים כ-views.

### ארבעה באגים שההרצה גילתה, לא הקריאה

1. יעדי ההפניה היו `/p/` ו-`/c/`. אף route באפליקציה לא עונה שם, ו-
   `sitemap.ts` מגיש לגוגל `/product/`. כל 301 שהצינור חישב הצביע ל-404.
2. היעד נבנה מ-`slug_decoded` ולא מ-`proposed_slug` שאחרי הדה-דופ.
   בהתנגשות, הכתובת הישנה של המפסיד הצביעה ל-slug של המנצח: הלקוח מגיע
   לדף של מוצר אחר ויכול לקנות את המוצר הלא נכון.
3. `_wp_old_slug` נקרא ונזרק. כל slug ישן עדיין מאונדקס ולא מופיע לא
   ב-sitemap ולא ב-permalink. ב-fixture: 2 כתובות מתוך 8.
4. `direct_match` היה קבוע false, כלומר `/product/x -> /product/x`.
   לולאה אינסופית שמורידה דף עובד.

### מיגרציה 095 הוחלה

`095_seo_redirects_and_wp_maps` על `ixvwfbuvfxxsjiywhbbb` דרך MCP.
‏`public.seo_redirects` היא החצי שחסר: 032 נתנה לצינור להחליט על הפניה,
ומעולם לא היה מאיפה ל-`src/proxy.ts` לקרוא אותה, כי `wp_import` הוא
service_role ולא חשוף ל-PostgREST.

חמישה CHECK, כולם נבדקו שהם באמת חוסמים (‏8/8 בשני הכיוונים), כולל
`no_self_loop` ו-`paths_are_relative` שחוסם `//evil.example.com`, כלומר
open redirect על הדומיין שלנו.
