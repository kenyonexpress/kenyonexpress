...
### 2026-07-29: סבב אימות שני על ה-Backlog (branch `arch/db-audit`)

כל 89 הקבצים נקראו שוב וכל שורה בטבלה נבדקה מול ה-DB החי בקריאה עצמאית. **אף
קביעה של הסבב הראשון לא נסתרה.** נוספו שלושה דברים.

**‏1. עמודות תלויות וסיכון בטבלת הסטטוס.** התלויות נגזרות מתוכן הקבצים ולא
ממספריהם, ולכן הן חושפות שרשראות שהמספור מסתיר: 064 חסומה על חמישה קבצים,
‏081 על 027 ועל 051, ו-051 על 027 בלבד.

**‏2. אימות ה-enum שנשאל במפורש.** `platform_settled` **אינו** ב-`order_status`
החי (pending, paid, partially_fulfilled, fulfilled, cancelled, refunded), והוא
כן ב-`settlement_status` וב-`payment_status`. זה לא פער: הערך מעולם לא היה מצב
של הזמנה אלא של שורת הזמנה ושל תשלום. אף מסלול כתיבה לא כותב אותו ל-`orders`,
והמופע היחיד במסלול הזמנה הוא מיפוי-קריאה ל-`split_executed` ב-
`src/server/queries/orders.ts`. מספר השורות שנושאות את הערך בפרודקשן: **אפס**,
ב-`order_items` וב-`payments` גם יחד.

**‏3. מיפוי הקוד מול 41 האובייקטים החסרים.** זו העמודה שחסרה מכל אודיט קודם:
לא מה חסר ב-DB אלא מה נשבר בגללו. מתוך 32 הקבצים החסרים, **בדיוק אחד גורם
לתקלה שמשתמש רואה**, והוא 027. מסך האדמין של התשלומים לספקים לא קורס אלא מציג
רשימה ריקה לנצח: הדף קורא `const { data, count } = await query` בלי לקרוא
ל-`error`, ו-`supabase-js` לא זורק על `42P01`, ולכן `data ?? []` הופך שגיאת
סכימה ל"אין תוצאות". אותו דפוס חוזר באנליטיקה (`fn_ingest_analytics_events`
חסרה, הקריאה נבלעת במכוון) וב-`settlement_events` מ-094. `src/lib/idempotency.ts`
הוא קוד מת עם אפס קוראים, וה-docstring שלו מפנה ל"מיגרציה 052" במקום ל-060.

**החלטה שהתקבלה אוטומטית:** לעדכן את `docs/MIGRATION-BACKLOG.md` הקיים במקום
ליצור מסמך שני. שני מסמכים שמודדים את אותו DB באותו יום הם בדיוק הבלבול שהאודיט
הזה נועד לחסל.

לא הורץ `apply_migration` ולא שום DDL. audit ודוקומנטציה בלבד.

---

### 2026-07-30: MEGA-6 עד MEGA-8, וארבעה באגים שנמצאו בהרצה

ארבעה ענפים, ‏20 קומיטים, שמונה מיגרציות שהוחלו דרך MCP ואומתו. ‏752 טסטים,
‏tsc ו-biome נקיים, בנייה עוברת.

| ענף | תוכן |
|---|---|
| `arch/mega-docs` | ששת מסמכי הארכיטקטורה, ‏4,731 שורות |
| `feat/wp-migration` | ‏MEGA-5 שלב 1, מיגרציה 095 |
| `feat/growth-core` | ‏MEGA-8 שלבים 1-4, מיגרציות 096-100 |
| `feat/observability` | ‏MEGA-7 שלב 1, וחצי הניגודיות של MEGA-6 |

**ארבעה באגים, כולם נמצאו בהרצה ולא בקריאה:**

1. **מפת ההפניות הצביעה ל-404.** היעדים היו `/p/` ו-`/c/`, ואף route לא עונה
   שם. `sitemap.ts` הגיש לגוגל `/product/` כל הזמן הזה. בנוסף היעד נבנה
   מה-slug שלפני הדה-דופ, כך שבהתנגשות שמות כתובת ישנה הצביעה לדף של מוצר
   אחר. זה גרוע מ-404: הלקוח מגיע לעמוד תקין וקונה את המוצר הלא נכון.
2. **`fn_claim_discount` הכשילה ניסיון חוזר של תשלום שהצליח.** בדיקת המגבלה
   פר-משתמש רצה לפני מקרה השידור החוזר, וראתה את הרשומה שאותה הזמנה יצרה.
3. **ריצה יבשה שכל שלביה נכשלו דיווחה על הצלחה ויצאה בקוד 0.** `run.fail()`
   בכוונה לא זורק, ו-`run.mjs` בדק רק זריקות. אף אחד לא שאל מה נרשם.
4. **טקסט לבן על צהוב ב-1.41:1** בכפתור ההגשה של כל ארבעת טופסי ההזדהות
   ובתג ההנחה. `globals.css` כבר הגדיר `--color-primary-foreground: #1a1a1a`
   עם ההערה "black on yellow". הטוקן היה נכון והרכיבים התעלמו ממנו.

**שתי טעויות שלי, ומה שתפס אותן.** טענתי ש-`#E4002B` נכשל בניגודיות לפני
שמדדתי; הוא 4.85:1 ועובר, והבדיקה נכשלה על הטענה שלי. וכתבתי
`instrumentation.ts` בשורש, שם Next מתעלם ממנו בשקט כשיש תיקיית `src/`.

### החלטות שהתקבלו אוטומטית

- **‏branch מ-`phase5/homepage` ולא מ-main.** ‏`cursor/add-supabase-3c830` הוא
  352 קומיטים מאחור ואין בו `supabase/migrations` בכלל. הוא אב קדמון טהור, אז
  יציאה מ-phase5 לא מאבדת דבר.
- **אין טבלאות `wp_product_map` / `wp_category_map`.** שתי העובדות כבר
  ב-`wp_import.id_map`. השמות קיימים כ-views מעל מקור האמת היחיד.
- **טבלת קמפיינים נפרדת מ-`public.coupons`**, שהיא supplier-scoped. עומס של
  שני סוגים על טבלה אחת מחייב ספק בקמפיין שהוא לא מימן.
- **`referral_program_settings` לא נזרעה.** התוכנית כבויה עד שאדם יזין כמה
  היא משלמת. לא הומצא סכום בונוס, מאותו נימוק ש-C1 אוסר ברירת מחדל.

### חוסם פתוח: MEGA-5 שלב 2

שני קלטים חסרים, שניהם מחוץ לשליטת הסוכן:

1. **קובץ ייצוא WordPress** ב-`ke-wpmig/ke-export.xml`. דורש התחברות לפאנל.
2. **מפתח `service_role` תקין.** ה-JWT ב-`.env.local` מפוענח ל-
   `iss: supabase-demo`, כלומר המפתח הציבורי של `supabase start`. הוא מעולם
   לא היה של `ixvwfbuvfxxsjiywhbbb`, וזו הסיבה ששלבי הכתיבה של הצינור מעולם
   לא רצו מול בסיס נתונים.

הצינור עצמו בנוי ונבדק: פרסר WXR, עץ קטגוריות, שחזור `_wp_old_slug`, ‏R2 עם
חמש נגזרות, ‏301 שומרי-slug, ומיגרציה 095 שכבר הוחלה.
