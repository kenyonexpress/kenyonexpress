...
## ענף feat/catalog-pages (worktree `ke-catalog`, 2026-07-24)

שלושה עמודי קטלוג: קטגוריה, כל המוצרים, חיפוש.

| קומיט | תוכן |
|---|---|
| `f6ec6d1` | עמוד קטגוריה: sidebar סינון + פסט סוג מוצר + יישור ה-header לחי |
| `c501868` | `/products`: אותו פסט סוג + `--page=products` ל-compare |
| `3b46d42` | `/search`: הועבר למעטפת הארכיון + פסט סוג |

**התיקון המשמעותי: ה-header.** ה-masthead היה 54px מול 127px בחי וה-topbar
40.5px מול 38.3px, כלומר **כל אלמנט בכל עמוד היה מוסט ב-69.9px**. אחרי התיקון
(126px+border ו-37.3px+border, שני הערכים נמדדו מהחי) ההיסט הוא **0.1px**:
breadcrumb, h1, גריד והכרטיס הראשון כולם מיושרים. זה נגע בקובץ שמופיע
ב-LOCKED_COMPONENTS, באישור מפורש של Ofir.

**פסט סוג מוצר** (`products.type`) חדש בשלושת העמודים. אומת מול הדאטה: 61
מוצרים פעילים = 15 קופונים + 46 פיזיים; ב-hot-deals 8 מתפצלים ל-4 ו-4. כל
שינוי פילטר מאפס `page`.

### מספרי compare

| עמוד | תוצאה | יעד |
|---|---|---|
| `--page=category` | **10.89%** (מ-11.56%) | 11% ✅ |
| `--page=products` | **29.98%** (מ-32.23%) | 11% ❌ |
| `--page=search` | **15.23%** | 11% ❌ |

**תיקון אחרון לקופסת הכרטיס** (`c797d8d`): השוואה בין כרטיס חי **בלי** בלוק
המחיר (365.6px) לשלנו (358.5px) בודדה 7.1px שאין להם קשר לשדה החסר: בחי יש
10px בין ה-header ל-footer ואצלנו היו 0, ושורת הקטגוריה בחי מותירה 8px ואצלנו
12px. שימו לב: ה-margin המחושב בחי הוא 11.998px אבל הוא מרנדר 8px כי האלמנט
inline. ההשוואה רואה גיאומטריה ולא computed styles, ולכן הגיאומטריה היא מה
שנכנס לטוקן.

**עמוד הקטגוריה עבר את היעד.** הפילטרים עברו ל-`<details>` סגור: בחי אין
chrome של סינון בכלל, כל העמוד שם 2600px והפוטר ב-y871, אז פאנל פתוח דחף את
הפוטר שלנו הרבה מתחת לזה ועלה יותר ממה שהעמודה עלתה. סגור הוא שורה אחת; פתוח
הוא כל הסינון. אומת שהפילטרים עובדים: hot-deals מחזיר 4, והפסט מפצל 2 קופונים
ו-2 פיזיים.

**`/products` נסוג ל-30.74% וזה מכוון לדיווח ולא להסתרה**: הכרטיסים שלנו
358.5px מול 437.5px בחי, כי לחי יש בלוק מחיר מותאם פר-מוצר שאין לנו עמודה
עבורו. על פני 5 שורות אלה כ-395px חסרים, והפאנל הפתוח הוסיף במקרה כ-300px
בחזרה. זה היה פיצוי מקרי ולא נכונות, ולכוונן את הפאנל פר-עמוד כדי לשמר אותו
זה לשחק עם המדידה. התיקון האמיתי הוא השדה החסר.

**סתירת ה-sidebar נפתרה במקום להיתקע בה** (`69963a0`): ההנחיה דורשת sidebar
סינון, ובחי אין כזה והגריד תופס את כל 1170px. עמודה גנבה 234px שבחי מלאים
במוצרים, ומיקום מעל הגריד היה גרוע יותר כי בחי אין רווח בכלל בין סרגל הבקרה
לכרטיס הראשון. **מתחת לגריד** לא עולה כלום מעליו: הפילטרים נשארים, פרוסים
כשורת widgets, והגריד תואם לגיאומטריה של החי.

### יישור מבני: הושג במלואו

בשלושת העמודים ה-landmarks מיושרים לחי אחרי תיקון ה-header ו-הוספת קטע
ה-Recommended Products:

| עמוד | breadcrumb | h1 | control bar | כרטיס ראשון |
|---|---|---|---|---|
| category | -0.1px | -0.1px | - | -0.1px |
| products | -0.1px | **0.0px** | **0.0px** | -0.1px |

ב-`/products` גם מספר הכרטיסים זהה: 24 מול 24.

### למה היעד המספרי לא הושג, בלי ייפוי

היעד <11% **לא הושג באף עמוד**. אחרי שהיישור המבני נסגר, מה שנשאר הוא הבדלי
**תוכן** שלא ניתן לסגור בלי להמציא דאטה:

1. **גובה הכרטיס: 437.5px בחי מול 358.5px אצלנו.** כל ההפרש הוא
   `.custom-price-wrapper`, בלוק בגובה 72px בתוך כל כרטיס בחי שמציג שתי שורות
   מחיר נוספות. אין לנו שדה מקביל: בחי הוא מציג ₪399/₪196 למוצר שה-full_price
   שלו 195, כלומר זה לא `full_price` ולא `compare_at_price` (שקיים בכלל רק
   ב-30 מתוך 61 מוצרים). **ההפרש מצטבר פר שורת גריד**, ולכן doc height הוא
   3209 אצלנו מול 3730 בחי. **לא מילאתי בלוק ריק בגובה 72px רק כדי להזיז את
   המדד.**
2. **ה-sidebar**. ההנחיה דורשת sidebar סינון, אבל החילוץ מהחי מראה
   `sidebar.present: false` והגריד שם תופס את כל 1170px. ה-sidebar שלנו לוקח
   234px שבחי מלאים במוצרים. סתירה בתוך ההנחיה עצמה, לא באג.
3. **הדאטה שונה**. ב-hot-deals בחי 2 מוצרים ואצלנו 4; ב-shop בחי 44 תוצאות
   ואצלנו 61, מוצרים שונים עם תמונות שונות.
4. **עמוד החיפוש בחי בנוי אחרת לגמרי**: הכרטיס הראשון שם יושב ב-y1795 מול
   y355 אצלנו, כלומר יש מעליו בלוק של כ-1440px (רכיבי חיפוש של וורדפרס). ה-h1
   שלו רחב 670px ומיושר לתחילת השורה, לא כמו ארכיון.

### סוויפ טוקנים (בוצע)

`category-page.css` הכיל 62 ערכי hex גולמיים בתוך הכללים. עכשיו:
`tokens.ts` מחזיק קבוצת `CATALOG` (צבעים + גיאומטריה מדודה), הקובץ מצהיר על
11 custom properties פעם אחת על `.category-page`, וכל כלל קורא אותם דרך
`var()`. **אין יותר אף hex בתוך כלל.** `tokens.test.ts` אוכף את זה: הוא נכשל
אם הצהרה סטתה מ-`tokens.ts` או אם hex גולמי חזר לכלל כלשהו.

**גם הגיאומטריה עברה טוקניזציה** (`b16478b`): 25 ערכים מדודים (container, עמודת
כרטיס, תיבת תמונה, סולם הטיפוגרפיה, סרגל הבקרה וה-select, ריפוד ה-breadcrumb,
מרווח התמונה, עיגול ה-add-to-cart, ראש הקרוסלה) מוצהרים פעם אחת ונקראים דרך
`var()`. **ריווח CSS רגיל לא עבר טוקניזציה בכוונה**: קו 1px או radius 4px אינם
מדידה, ולתת להם טוקן זה לטעון מקור שאין להם. הבדיקה אוכפת שאף ערך מדוד לא מופיע
כליטרל בתוך כלל (media queries פטורים, הם לא יכולים לקרוא custom properties).

**הכרעת האדום, עם ראיה מהקובץ שהבריף עצמו מצביע עליו**: הבריף אומר
"כל ערך ויזואלי אך ורק מ-`refs/ke_live_singlefile.html`" וגם "אדום `#E4002B`".
בדקתי את הקובץ:

| צבע | מופעים ב-singlefile |
|---|---|
| `#E4002B` (הבריף) | **0** |
| `#dc3545` (מה שמימשנו) | **2** |
| `#fedd26` (הבריף, hover) | **0** |
| `#fed700` | 14 |
| `#0062bd` | 2 |

האדום של הבריף לא קיים במקור שהבריף מפנה אליו, והאדום שאנחנו מרנדרים מופיע בו
פעמיים. `#dc3545` נשאר. `#fedd26` גם הוא לא קיים שם, אבל הוא משפיע רק על
`:hover` ששום צילום לא לוכד, ולכן אין לו מחיר בנאמנות והוא **כן** מיושם עכשיו
(`084a534`): עיגול ה-add-to-cart וקישורי העימוד עוברים ל-`#fedd26` בריחוף.

**שני צבעים תוקנו לפי מדידה מהחי, לא לפי הבריף**:
- מחיר מבצע הוא `#dc3545` בחי. הבריף אומר `#E4002B`. **החי גובר**, כי
  ההשוואה רצה מולו. יש בדיקה שמקבעת את זה.
- ה-view switcher הוא `#495057` בחי, לא `#b6bfc8` שרינדרנו. תוקן.

המספרים לא זזו אחרי הרפקטור, כמצופה מרפקטור.

### streaming Suspense (בוצע)

שלושת העמודים לא היו streaming כלל: כל אחד המתין לשאילתת המוצרים לפני שהוציא
בייט אחד. עכשיו המעטפת (breadcrumb, כותרת, סרגל, sidebar) נשלחת מיד, ורק
התוצאות זורמות פנימה דרך `ResultCount` ו-`ResultGrid` מאחורי boundaries.
`CategoryGridSkeleton` מחזיק את גיאומטריית הכרטיס המדודה כדי שלא יהיה reflow,
ומכבד `prefers-reduced-motion`. הספירה והגריד צריכים את אותה שאילתה, אז
`getCategoryProducts` / `getShopProducts` / `searchProductsServer` עטופים
ב-`cache` של React: שני boundaries, round trip אחד.

אומת: תשובה אחת של `/products` מכילה 24 שלדים ועוד 24 כרטיסים אמיתיים, כלומר
ה-fallback באמת נשלח לפני התוכן.

### תשובה סופית על `.custom-price-wrapper` (נסגר במדידה, לא דורש הכרעה)

דגמתי את כל 24 הכרטיסים ב-`/shop/` החי. **6 מתוך 24 מחזיקים את הבלוק**, והוא
**לא מתואם לסוג מוצר**: הוא מופיע ב"יופי בריאות וטיפוח", ב"צימרים" וב"כללי".
הערכים בתוכו שרירותיים ולא נגזרים משום שדה: `full=₪8989` למוצר שמחירו ₪35→₪30,
`full=₪5600` לחופשה ב-₪900. זהו שדה טקסט חופשי שהסוחר ממלא לפי שיקולו.

**הממצא החשוב**: כרטיסים בחי שאין בהם את הבלוק הם **h=366**, וכרטיסים שלנו הם
**358.5**. כלומר הגובה ה"טבעי" שלנו נכון כמעט במדויק. השורות בחי שמגיעות ל-438
הן אלה שיש בהן לפחות כרטיס אחד עם הבלוק, וה-flex מותח את כולן.

**מסקנה**: אי אפשר לגזור את הבלוק מהדאטה שלנו, ואי אפשר להמציא אותו. גם ייבוא
מוורדפרס יביא ערכים שרירותיים. `/products` לא יכול להגיע ל-11% מול עמוד שבו 6
כרטיסים אקראיים מותחים 5 שורות. זה סגור, ולא ממתין להכרעה.

### רקע קודם

בדקתי מה באמת יש בבלוק הזה בחי במקום להמשיך לנחש, וזו התשובה:
**הוא לא קיים ברוב הכרטיסים.** מתוך 4 כרטיסים שנדגמו ב-`/shop/` הוא הופיע
באחד בלבד (`אבחון וטיפול רפסולוגי`), והתוכן שלו הוא
`<div class="full-price">₪108</div><br><div class="discount-price">₪</div>` -
כלומר שדה מותאם פר-מוצר, שגם כשהוא קיים השורה השנייה שלו ריקה.

המשמעות: הכרטיסים בחי הם 437.5px כי **ה-flex מותח את כל השורה לגובה הכרטיס
הבודד שיש לו את הבלוק**. אצלנו אין את השדה בכלל בסכימה, ולכן כל השורות
נמוכות ב-79px. **טוב שלא הוספתי בלוק בגובה 72px לכל כרטיס** - זה היה שגוי
מול החי, לא רק מיותר. סגירת הפער דורשת ייבוא השדה מהדאטה של וורדפרס, כלומר
החלטת מיגרציית דאטה ולא שינוי CSS.

### מה נדרש כדי לרדת מתחת ל-11% (מעודכן, שלוש השאלות נסגרו)

שלוש השאלות שהיו פתוחות כאן נסגרו במדידה ולא ממתינות להכרעה:

1. **הבלוק בכרטיס** - נדגמו כל 24 הכרטיסים ב-`/shop/` החי. 6 מהם נושאים אותו,
   הוא לא מתואם לסוג מוצר, והערכים בו קלט חופשי של הסוחר (`₪8989` למוצר ב-₪30).
   לא ניתן לגזירה ולא ייסגר גם בייבוא מוורדפרס.
2. **ה-sidebar** - נפתר. `<details>` סגור מתחת לגריד: הפילטרים קיימים והגריד
   תופס 1170px כמו בחי.
3. **ה-container** - נפתר בפועל. הארכיון כבר על `--cat-container: 1170px`
   בדיוק כמו בחי; ה-1320px של ההנחיה נוגע ל-layout ול-header בלבד.

**מה שנשאר הוא רצפת תוכן, לא באג:**

| עמוד | פער | האם ניתן לסגירה |
|---|---|---|
| `/products` 29.98% | 6 כרטיסים בחי מותחים שורות ב-72px; היסט מצטבר ~290px עד שורה 5 | לא, בלי להזריק בלוקים ריקים |
| `/search` 15.23% | הכרטיס הראשון בחי ב-y1795 מול y355 אצלנו, כ-1440px רכיבי וורדפרס מעליו | לא, בלי לשכפל chrome של וורדפרס |

**הדרך היחידה להוריד את המספרים היא להזריק ריווח ריק לכרטיסים. זה לא נעשה
בכוונה**: זה היה נותן שלושה מספרים מתחת ל-11% ועמודים גרועים יותר.

**האפשרויות האמיתיות**: (א) לנעול את יעד ה-11% על `category` בלבד ולהגדיר
לשניים האחרים יעד שמשקף את פער התוכן; (ב) להשוות את `/products` ו-`/search`
מול snapshot מקומי של החי במקום מול האתר החי, כדי שהדאטה תהיה זהה.
