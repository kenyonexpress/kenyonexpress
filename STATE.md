...
### ⚠️ שתי מדידות הרוחב הקודמות **פסולות** — הכיתה לא נוצרה
בדיקה אחרונה גילתה שכשמשנים את `md:grid-cols-[5fr_7fr]` לערך אחר,
‏Tailwind **לא מייצר** את הכיתה החדשה ב-build, והגריד נופל לעמודה אחת.
כלומר שני הניסויים שדיווחתי כ"מחמירים" (‏`470fr_700fr` ו-`36fr_64fr`)
מדדו layout שקרס, לא יחס אחר. ‏`5fr_7fr` המקורי **כן** נוצר ותקין:
`.md\:grid-cols-\[5fr_7fr\]{grid-template-columns:5fr 7fr}`.

**שאלת הרוחב עדיין פתוחה.** מי שממשיך: לוודא ראשית שהכיתה החדשה מופיעה
ב-`.next/static/chunks/*.css` לפני שמאמינים למספר.

### הניסוי הפסול (נשמר לתיעוד)
...
4. ✅ **‏E2E: ‏53/53 ירוק, 0 דולגו.**
   בסבב הזה הסוויטה הייתה אדומה זמנית על ספקי העגלה, וזו **הייתה תחרות על
   ה-DB ולא רגרסיה**: `fullyParallel` עם workers לא חסומים הריץ ~חצי
   מהליבות, וכל worker כתב לעגלת אורח מול **אותו** פרויקט Supabase.
   מספר העוברים נדד בין ריצות (‏53, ‏50, ‏44) והספק שנכשל עבר לבד ב-8.7
   שניות — חתימה של תחרות, לא של באג דטרמיניסטי.
   ‏`playwright.config.ts` מגביל עכשיו ל-2 workers מקומית
   (‏`E2E_WORKERS` דורס למי שמריץ מול DB מקומי). ‏CI כבר היה על 1.
