...
(דף המוצר סטטי). הבא בתור: **goal 20 (Final)**, ולפניו למדוד את דף המוצר
**מחוץ למחשב הזה** (ראה [42]).
...
נמדד ב-[41]. התיקון בוצע ב-[42] ו-**L1 עובר ב-40 VU** (‏product p95 657ms
מול סף 800). ‏**ב-200 VU המלאים הוא עדיין נופל** ‏(p95 44.8s), וזה מחייב
מדידה מחוץ למחשב הזה לפני שנקבע אם זו הארכיטקטורה או הלפטופ.
