...
**‏1. 🔴 החסם היחיד שנשאר בנתיב הרכישה: `public.vouchers` לא קיימת.**
`finalize.ts:307` קורא ל-`issueVouchersForItem`, שקורא וכותב ל-`vouchers`,
**לפני** שהוא מגיע לכתיבת `settlement_status`. לכן מסלול הקופון נופל מוקדם
יותר מהבאג של ה-enum שכבר תוקן. ליצור אותה = שאר מיגרציה 054, שתלויה
ב-027 (`supplier_members`) שגם היא חסרה. זה החסם הבא וגדול משקדמו לו.
**המצב היום: אפשר לקנות קופון, אי אפשר לסגור את ההזמנה.**
