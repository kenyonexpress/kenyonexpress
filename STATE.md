...
היקף: **בלי `cacheComponents` אין תיקון חלקי**. גבול Suspense לבדו לא עוזר,
כי בלי PPR כל נגיעה ב-`cookies()` בכל מקום בעץ מוציאה את ה-route מהסטטי גם
אם היא עטופה. כלומר זו מיגרציה של האפליקציה כולה (‏`use cache` או Suspense
על כל שליפת נתונים בכל route, כולל אדמין וצ'קאאוט), ולא דגל. זה הסבב הבא.
