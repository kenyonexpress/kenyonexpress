...
## Next Task
מה ש-[47] השאיר מדוד ולא עשה, קודם: ‏**69 מתוך 74 ה-Server Functions עדיין בלי
‏request-id** (חמש מסלול-הכסף חוברו). ‏`app/error.tsx` נשאר על console כי הוא רץ
בדפדפן, וזו החלטה ולא פער. ‏**מה שלא הצלחתי לאמת חי כאן ונרשם ככזה:** שורת לוג
...
### 2026-08-03: [45] שישה באקטים ציבוריים הפסיקו להיות ניתנים לרישום

הראשון משני הממצאים ש-[44] פתח ולא טיפל בהם. נמדד ולא נקרא מהלינט:
כל ששת הבאקטים ציבוריים **וגם** נושאים מדיניות SELECT על `storage.objects`
לתפקיד `public` שכל תוכנה `bucket_id = '<שם>'`. עם המפתח הפרסומי בלבד:

```
POST /storage/v1/object/list/product-images {"prefix":"products"}
  -> 200, עשרה אובייקטים עם שם וגודל בבייטים
```

**אותה צורה כמו 104: סמוי, לא חי.** עשרת האובייקטים היום הם תמונות מוצר
בשמות UUID שממילא ציבוריות, ולכן שום דבר לא דולף כרגע. זה נורה ברגע שיגיע
לבאקט ציבורי משהו שנועד להיות נגיש רק למי שמחזיק את הכתובת שלו.

**התיקון לא נוחש, הוא נמדד לפני שנכתב.** עם `product-images: public read`
מוסרת, מול הפרודקשן:

| בקשה | לפני | אחרי |
|---|---|---|
| `GET /object/public/product-images/<path>` | 200, ‏15114B | **200, ‏15114B** |
| `POST /object/list/product-images` | 10 אובייקטים | **`[]`** |

באקט ציבורי מגיש את `/object/public/...` **בלי להתייעץ עם RLS בכלל**, וזו
בדיוק הסיבה שהמדיניות לא עלתה שם כלום. `getPublicUrl()` ב-
‏`src/lib/storage/upload.ts:54` בונה את המסלול הזה, והוא היחיד שממנו האתר
מצייר. ‏4 מתוך 4 כתובות ציבוריות שנבדקו נשארו 200.

**‏403 שכמעט נספר כרגרסיה:** מסלול הטרנספורמציה
‏`/render/image/public/...` מחזיר 403, אבל הגוף הוא
‏`{"error":"FeatureNotEnabled","message":"feature not enabled for this tenant"}`,
כלומר מגבלת תוכנית ולא דחיית RLS, ולמסלול הזה **אפס אזכורים בריפו**.
בלי לקרוא את הגוף זה היה נרשם כנזק של השינוי.

**המדיניות לא נמחקה, היא צומצמה.** נתיב המחיקה של Supabase Storage קורא את
השורה שהוא עומד להסיר, ולכן כל באקט מקבל SELECT בחזרה **תחת בדיוק הפרדיקט
של מדיניות ה-DELETE שלו**: מי שיכול למחוק מבאקט יכול לרשום אותו, ואף אחד
אחר, כולל לקוח מחובר. `vendor-logos` קיבל `has_role('vendor')` בלי
‏`is_admin()`, כי זה מה שכתוב במדיניות הכתיבה שלו: מראה, לא שיפור.

אחרי ההחלה, ‏6 מתוך 6 באקטים מחזירים `[]` ל-anon, והיועץ של Supabase כבר
לא מדווח על אף `public_bucket_allows_listing`. מיגרציה `105`, הוחלה דרך MCP.

**מה שנשאר מ-[44] ואינו ניתן לביצוע מכאן:** `auth_leaked_password_protection`
הוא מתג בהגדרות Auth ולא DDL, ואין לו כלי ב-MCP. הוא של אופיר.
