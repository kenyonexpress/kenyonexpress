...
אותה בנייה: ‏**1120 / 884 / 751ms** מול סף 800. הסף יושב **בתוך** רצועת
הרעש של הלפטופ, ולכן "עובר" היה הגרלה אחת ולא מדידה. ‏**ב-200 VU המלאים
הוא נופל בבירור** ‏(p95 44.8s ב-[42]).
‏**הליד הבא, מדוד ב-[43]:** ‏`home` הוא ‏min 3ms, אבל `product` ו-`category`
שניהם ‏**min ~238ms**, כלומר רצפה קבועה פר בקשה שאינה עומס ואינה הקטלוג
(הוא כבר מאחורי `use cache` מ-[42]). זה החור הדינמי של PPR, וזה מה שיש
למדוד הלאה, מחוץ למחשב הזה.
