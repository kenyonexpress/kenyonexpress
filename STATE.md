...
| `--page=products` | **32.23%** | 11% |
| `--page=search` | לא הורץ, אין URL מקביל מוגדר בחי | - |

### למה היעד לא הושג, בלי ייפוי

היעד <11% **לא הושג**, וזה לא עניין של CSS. שלוש סיבות מבניות:

1. **ה-sidebar**. ההנחיה דורשת sidebar סינון, אבל החילוץ מהחי מראה
   `sidebar.present: false` והגריד שם תופס את כל 1170px. ה-sidebar שלנו לוקח
   234px שבחי מלאים במוצרים. זו סתירה בתוך ההנחיה עצמה, לא באג.
2. **הדאטה שונה**. ב-hot-deals בחי 2 מוצרים ואצלנו 4; ב-shop בחי 44 תוצאות
   ואצלנו 61, ומוצרים שונים עם תמונות שונות.
3. **`/shop/` בחי מכיל קרוסלת "Recommended Products"** בין ה-breadcrumb ל-h1,
   ובקרת "Show 20" + pager מספרי בסרגל. אין לנו אותם, וזה מסיט את כל מה שמתחת.
