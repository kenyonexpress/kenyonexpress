...
| קובץ | תוכן | סטטוס |
|---|---|---|
| `052_product_approval_workflow.sql` | משוחזר מהענף הישן כפי שהוחל על ה-DB (verbatim) | חובה לפריטת parity |
| `053_admin_rbac_support.sql` | תפקיד support (enum + `is_support()` + מדיניות SELECT מפורשות), טבלאות affiliates + referrals (לפי צורות 010), `v_admin_pending_queues` מעל טבלאות חיות בלבד | חדש |
