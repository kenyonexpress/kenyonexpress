-- ============================================================================
-- 051_money_integer_units.sql  (spec number 034; renumbered 033->050 ...
...
-- recreated in the new units (platform_price_agorot, discount_bp).
--
...
DO $$ BEGIN
  IF to_regclass('public.coupon_deals') IS NOT NULL THEN
    -- Same business meaning as the 015 columns, in the new units:
    -- platform price = 10% of face, discount at the business = 90% = 9000 bp.
    ALTER TABLE public.coupon_deals
      ADD COLUMN IF NOT EXISTS platform_price_agorot integer
        GENERATED ALWAYS AS ((round((original_price_agorot)::numeric * 0.10))::integer) STORED;
    ALTER TABLE public.coupon_deals
      ADD COLUMN IF NOT EXISTS discount_bp integer
        GENERATED ALWAYS AS (9000) STORED;
  END IF;
END $$;
