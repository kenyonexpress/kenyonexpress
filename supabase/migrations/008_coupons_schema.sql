...
CREATE UNIQUE INDEX IF NOT EXISTS idx_coupon_codes_code
  ON public.coupon_codes (code);
CREATE INDEX IF NOT EXISTS idx_coupon_codes_user_status
  ON public.coupon_codes (user_id, status);
CREATE INDEX IF NOT EXISTS idx_coupon_codes_order_item
  ON public.coupon_codes (order_item_id);
CREATE INDEX IF NOT EXISTS idx_coupon_codes_supplier_status
  ON public.coupon_codes (supplier_id, status);
CREATE INDEX IF NOT EXISTS idx_coupon_codes_expires_active
  ON public.coupon_codes (expires_at) WHERE status = 'issued';
...
-- Users SELECT their own coupons
...
-- Suppliers SELECT coupons assigned to them
...
-- Suppliers UPDATE only used_at/used_by/scan_method/status on their own coupons
