...
BEGIN
  IF to_regclass('public.analytics_events') IS NULL
    ...
    RAISE EXCEPTION '056_analytics_v3 requires 033_analytics (analytics_* objects missing)';
  END IF;

  IF to_regproc('public.fn_il_date') IS NULL THEN
    RAISE EXCEPTION '056_analytics_v3 requires 033_analytics (fn_il_date missing)';
  END IF;
END $$;
