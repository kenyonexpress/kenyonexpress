...
| R-8 | Daily recon never runs (scheduler absent) | **Certain** | Critical | D-8 |
...
**Live.** There is no Hono process and no Workers app in the repo. DNS and R2 are the Cloudflare surface. Adding Workers without this discipline is how R-1 becomes an incident.
...
**Live.** Child tables do not exist. `product_type` on `products` is the discriminator with nullable money on the same row. Moving to CTI without a backfill is R-2 on day one.
...
**Live.** Meilisearch is unset in production. Drift is currently "there is no index". Turning Meili on without the checker is how R-7 goes silent.

---

## R-8 Daily recon never runs

**Why it exists.** Decision 8 accepts hours of lag **if a job runs**. Live: ten cron routes, nothing calls them (`docs/CRON-EXTERNAL.md`, `docs/FAILURE-MODES.md` §2.1). Vouchers do not expire. Escrow would never release. `verified_against_api AND processed_at IS NULL` would never page from a schedule.

**What breaks.** Missed webhook sits forever. Expiry credits never land. This is **certain** today, independent of escrow.

**Mitigation.** External scheduler (or Vercel cron) on `/api/cron/reconcile` and the rest of the money jobs. `/api/ready` does not replace a clock. A recon job that is only documented is R-8.

**Live evidence (2026-09-09).** `vercel.json` in `kenyonexpress` has no `crons` key. Ten `/api/cron/*` routes exist. Nothing in the Vercel project config calls them.
...
**Mitigation.** httpOnly + Secure + SameSite=Lax. Rotation on refresh. Logout clears cookies. Do not put the session in `localStorage`. Guest cart cookie follows the same flags so analytics cannot forge another visitor's id.
...
**Why it exists.** Decision 7 puts enforcement in Postgres. `payment_webhook_events` is supposed to be server-insert only: the replay key, the raw Cardcom body, sometimes a token. Migration 172 added `payment_webhook_events_admin_read` because the admin payments tab used the user-scoped client and, under deny-all, showed an empty table. The tab now works. The grant is SELECT of the **raw row**.
...
**Mitigation.** Keep writes service-role only (already true). Replace the table grant with a SECURITY DEFINER that returns the columns an operator needs (`external_event_id`, status, amounts, timestamps) and redacts tokens. Until then, treat 172 as a known live exception, named in the `ARCHITECTURE.md` header, not as the contract.

**Live.** Certain. 172 is applied. `docs/DB-SECURITY-MODEL.md` still describes the pre-172 zero-policy set for this table; do not trust that paragraph for grants.
