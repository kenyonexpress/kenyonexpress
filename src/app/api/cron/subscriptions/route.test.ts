...
/**
 * Auth gate for the subscription biller. It charges saved cards through
 * Cardcom; an anonymous trigger would bill customers on someone else's clock.
 */

const { createAdminClient, getPaymentProvider } = vi.hoisted(() => ({
...
function request(auth?: string): NextRequest {
  return new NextRequest('https://example.test/api/cron/subscriptions', {
    headers: auth ? { authorization: auth } : {},
  })
}

describe('subscriptions cron auth', () => {
  beforeEach(() => {
