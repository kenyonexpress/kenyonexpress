 ...
 * single exported constant does not fail on itself: it voids EVERY export in
 * the file, so both newsletter pages fail to build with "the module has no
 * exports at all". Nothing outside this file reads it. Note the unrelated
 * `CONSENT_WORDING_VERSION` in `src/lib/analytics/consent.ts`, which is the
 * analytics banner's version number and is a different fact entirely.
 */
const CONSENT_WORDING_VERSION = 'newsletter-v1'
