 ...
 * recording.
 *
 * Consent: same cookie, same window event, same shape as ThirdPartyTags. A
 * declined banner means no recorder is ever downloaded, not a recorder that
 * politely does not record. Recording stops meaning anything without masking:
 * all inputs are masked, so a card form or a password field records as
 * asterisks even if a future refactor forgets this file exists.
 */
export default function PostHogReplay() {
  ...
  useEffect(() => {
    ...
    const check = () => setAllowed(isTrackingAllowed(readCookie()))
    check()
  ...
  useEffect(() => {
    if (!allowed || !isPostHogEnabled()) return
    if ((window as unknown as { __ke_posthog?: unknown }).__ke_posthog) return
