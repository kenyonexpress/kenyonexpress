    ...
    <form method="get" action={basePath} className="flex flex-wrap items-center gap-3">
      ...
      <div className="relative min-w-[12rem] max-w-sm flex-1">
        ...
        <input
          ...
          className="h-9 w-full rounded-md border border-black/10 bg-surface ps-9 pe-3 text-sm text-ink outline-none focus-visible:ring-2 focus-visible:ring-brand-primary"
        />
      </div>
      {children}
      <button
        type="submit"
        className="h-9 rounded-md bg-heading px-4 text-sm font-medium text-white transition-colors hover:bg-black"
      >
        סינון
      </button>
    </form>
