...
describe('buildCartView: a type the cart cannot price', () => {
  ...
  it('shows an all-unpriceable cart as unavailable lines, not as an empty cart', () => {
    // Merge note (2026-08-19): this test arrived from feat/supplier-portal
    // asserting `items` length 0, which is the EMPTY_CART regression the test
    // at "still shows the lines when not one of them can be priced" exists to
    // pin as FIXED. The branch was written before that fix landed.
    //
    // The intent it was really protecting survives untouched and is asserted
    // below: a service line is never sold, and contributes no money. What
    // changed is that the customer now SEES the row they put in their cart,
    // with the reason, instead of being shown an empty cart over a stored line.
    const cart = buildCartView('cart-1', [stored()], [product({ type: 'service' })], [])

    expect(cart.items).toHaveLength(1)
    expect(line(cart).available).toBe(false)
    expect(line(cart).unavailable_reason).toBe('unpriced')
    // The part that must never regress: no money for a line the engine never saw.
    expect(cart.total).toBe(0)
    expect(cart.subtotal).toBe(0)
    expect(cart.platform_fee).toBe(0)
    expect(line(cart).platform_fee).toBe(0)
    expect(line(cart).supplier_due).toBe(0)
  })
