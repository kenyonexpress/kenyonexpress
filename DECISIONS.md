...
> **עדכון 19.08, אחרי המעשה:** סוכן אחר ביצע בינתיים את שני המיזוגים *ישירות*
> אל `phase5/homepage` (`c6942ee58`, `f8f6c9d14`), והם עברו. הענף
> `merge/supplier-and-arch-night` הפך לכן **מיותר**, והוא נשאר דחוף כראיה בלבד.
>
> **זה לא הופך את ההכרעה שלמטה ללא נכונה, והנתונים שבה נמדדו ולא שוערו.**
> הסיכון היה אמיתי גם אם הפעם הוא לא התממש. שווה לציין את מה שכן קרה:
> שני סוכנים פתרו את התנגשות `pricing.ts` **באופן זהה פונקציונלית ובאופן
> עצמאי** — אותו שומר `type != null`, אותו `available: reason === null`, אותה
> הכרעה לטובת "עגלה שאי אפשר לתמחר אינה עגלה ריקה". ההסכמה הבלתי-תלויה הזאת
> היא הראיה החזקה ביותר שהפתרון נכון.
>
> ההנמקה שנכתבה בסוף ב-`pricing.test.ts` על ה-trunk **טובה משלי**: היא מונה שתי
> תוצאות מעשיות שלא ניסחתי — הוספת פריט מחדש שמצטרפת בשקט לשורה שהייתה שם כל
> הזמן, והבאנר שמציע לנקות שורות לא-זמינות שלעולם לא יכול להופיע דווקא כשכל
> העגלה לא זמינה.
