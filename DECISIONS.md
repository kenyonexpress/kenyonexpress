...
## ‏D13: איחוד ה-RLS פוצל לפי תפקיד ולא מוזג ל-`public`, ‏19.08.2026

‏שלוש עשרה `multiple_permissive_policies` נסגרו במסגרת (26). המסמך שקדם להן
תיאר את המדיניות ה-`_authenticated` בכל זוג כ"חסרת השפעה", **וזה נמדד כשגוי**:
כל זוג נשא שני ענפי הרשאה שונים.

**ההחלטה שהתקבלה לבד:** לא למזג לתוך מדיניות אחת `to public`, אלא לפצל לפי
תפקיד בארבע הטבלאות ש-`anon` קורא. הסיבה נמדדה ולא הונחה: ‏`anon` אינו מחזיק
‏`EXECUTE` על `current_user_role()` ועל `has_role(text)`, ותחת RLS שגיאת
הרשאה ב-predicate מפילה את כל השאילתה ולא מסננת שורה. המיזוג הראשון החזיר
‏`42501` על קריאת הקטלוג של מבקר לא מחובר.

**מה שנשמר:** ההרשאה עצמה זהה לפני ואחרי, ואומתה עם `set local role anon`
ועם תפקיד `authenticated` נושא `sub` אמיתי.

**ממתין לאישור אופיר:** לא. זה תיקון ביצועים בלי שינוי הרשאה.

## ‏D14: ה-CHECK של `notification_outbox` הורחב במקום להתבטל, ‏19.08.2026

ה-constraint אישר ‏5 סוגי הודעה בזמן שהקוד שולח ‏10, וחמש התראות היו מתות
בפרודקשן. שתי דרכים לתקן: לבטל את ה-CHECK, או להרחיב אותו.

**ההחלטה שהתקבלה לבד: להרחיב.** ‏CHECK הוא מה שגורם לסוג עם שגיאת כתיב להיכשל
ברעש ב-INSERT, במקום להחנות בתור שורה שה-drain לעולם לא יוכל לרנדר. הרשימה
המורחבת היא בדיוק מה ש-`buildNotification` יודע לרנדר, ו-
`src/lib/email/outbox-kinds.test.ts` נופל אם הן יתפצלו שוב.

**ממתין לאישור אופיר:** לא. זה תיקון באג.

## ‏D15: ‏`welcome` ו-`refund_completed` נכתבו בלי טריגר ב-DB, ‏19.08.2026

הרישום הקודם ל-(21) אמר ששתי התבניות החסרות חסומות, כי כל סוג הודעה חדש דורש
טריגר, שהוא migration על פרודקשן. **שתי הטענות נמדדו כשגויות:** ההכנסה לתור
נעשית מקוד האפליקציה בכמה מקומות קיימים, ו-`kind` הוא `text`.

**ההחלטה שהתקבלה לבד:** להכניס לתור מקוד האפליקציה, ולהשעין את ה"פעם אחת"
של מייל ברוך הבא על `dedupe_key` ה-UNIQUE של התור ולא על דגל בפרופיל.
‏ה-callback רץ בכל התחברות ורק הראשונה יוצרת שורה. אידמפוטנטיות במקום אחד
עדיפה על דגל שמישהו צריך לזכור לעדכן.

**ממתין לאישור אופיר:** נוסח שני המיילים בלבד. מייל הזיכוי **לא נוקב בתאריך**
בכוונה, כי מי שקובע מתי הכסף מופיע הוא מנפיק הכרטיס.
