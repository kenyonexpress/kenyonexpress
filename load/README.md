...
| `spike.js` | ‏L8 קפיצה פי ‏10 ל-60 שניות, שלושה שלבים מתויגים | לא | **כן** |

כל תרחיש רושם ‏`med` (‏p50), ‏`p(95)`, ‏`p(99)` ו-`max` דרך
‏`lib/summary.js`, ולא רק את ברירת המחדל של ‏k6 שאין בה ‏p99. הסיכום של
‏`--summary-export` הופך לטבלה שב-`docs/LOAD-TEST-RESULTS.md` עם:

```
node scripts/load-summary.mjs summary.json
```

**שימו לב לכיוון של הבוליאנים ב-JSON של ‏k6:** ב-`thresholds` של כל מדד,
‏`true` פירושו שהסף **נחצה**. הסקריפט קורא אותם נכון; קריאה ידנית נוטה להפוך.
...
```
LOAD_BASE=http://localhost:3411 LOAD_BASELINE=5 LOAD_SPIKE_FACTOR=10 k6 run load/spike.js
```

‏30 שניות של ‏5 ‏VUs, קפיצה מיידית ל-50 למשך ‏60 שניות, וחזרה ל-5 ל-30 שניות.
כל בקשה מתויגת ‏`phase:baseline|spike|recovery`, ולכן שלושת השלבים מקבלים ‏p95
משלהם ושער משלהם: הבסיס וההתאוששות נמדדים מול יעד עמוד המוצר (‏800ms), והקפיצה
מול נקודת הכשל שלו (‏1500ms). **ההתאוששות היא השער שחשוב:** אתר שאיטי בזמן
הקפיצה ועדיין איטי דקה אחריה סובל מדליפה, לא ממגבלת קיבולת.
