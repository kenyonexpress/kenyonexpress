...
006 had and this did not were folded in rather than lost: the `stage` column (the
same token `capturePaymentAlarm` tags Sentry with, so an alarm joins to its rows),
`external_event_id`, `provider`, and a wider event vocabulary. What was **not**
taken is 006's `text` + CHECK approach to the event type: an enum enforces the same
closed set, compares more cheaply, and appears in the generated types.
