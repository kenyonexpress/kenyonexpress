...
### החלטה ארכיטקטונית: אחוז הספק **נגזר**, לא נשמר

**עודכן 2026-07-27 בעת המימוש.** הנוסח הקודם כאן הציע לשמור את שני האחוזים
ולכפות `platform_percent + supplier_split_percent = 100` ב-constraint.
זה נדחה בזמן הכתיבה לטובת פתרון פשוט יותר: **`platform_percent` הוא העמודה
היחידה, ואחוז הספק מוצג חי בטופס כ-`100 − platform_percent` בלי להישמר.**

הנימוק: constraint מונע שורה סותרת, אבל עמודה שנייה שחייבת תמיד להיות
המשלים של הראשונה היא יתירות שאין לה תועלת. גזירה לא יכולה לסטות בכלל,
לא דורשת מיגרציה, ולא דורשת שמירה על סנכרון בשני מסלולי כתיבה.
אופיר עדיין רואה את שני המספרים בטופס, שזו הייתה הדרישה.

מומש ב-`src/components/admin/ProductForm.tsx` ובסכמה של
`src/server/actions/admin/products.ts` (השדה **חובה**, אין ברירת מחדל).
