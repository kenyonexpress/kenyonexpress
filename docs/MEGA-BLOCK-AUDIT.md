...
## מגה-בלוק 14 (‏STEPS 78–82) — נגישות עמוקה: רובו קיים, ‏81 נבנה ותפס (02.09)

**קיים מגל הנגישות:** ‏e2e/a11y.spec.ts — ‏axe WCAG A/AA אפס-הפרות על 19
מסלולים ציבוריים, בשני הפרויקטים (78); בדיקות reflow ב-320/640/1440,
מסמך he+rtl, ‏skip link, ‏contrast מתועד עם ראיות מדודות (‏priceStrike
הוכהה ל-5.02:1) (79–81 חלקית); ‏keyboard.spec נפרד לא נדרש — האינטראקציות
מכוסות בספקי הזרימה.

**‏81 נבנה ותפס באמת:** ‏e2e/touch-targets.spec.ts (‏WCAG 2.5.8, רצפת
24px + ‏40px לכפתור הקנייה, פטור לקישורי-פרוזה ולתבנית clip של skip-link).
ההרצה הראשונה מצאה **10 מטרות קטנות מדי** בנתיבי הליבה: אייקוני
לב/חשבון (‏22px), טריגר העגלה, נקודות ה-hero (‏16px רוחב), תאי
‏CategoryStrip טקסטואליים (‏19px גובה) וקישורי פירורי-לחם. כולם תוקנו
בריפוד ניטרלי-layout (‏-m-1 p-1, ‏max-lg:min-h-11, ‏py-1) — שער ה-1440
נמדד מחדש: ‏8.3%, לא זז.

**‏82:** תאריך הצהרת הנגישות עודכן ל-02.09.2026 (המגבלות הידועות נשארות
נקובות, כולל היעדר ביקורת חיצונית).

## מגה-בלוק 15 (‏STEPS 83–87) — חוב ותיעוד (02.09)

- **83:** מלאי קוד מת ב-`docs/DEAD-CODE.md` — ‏drizzle-orm/kit + ‏src/db +
  ‏drizzle.config.ts מתים מוכח; **המחיקה עצמה מחכה לאופיר** (עצירה קשה),
  עם פקודות מוכנות. ‏node-forge/qrcode/recharts נבדקו — חיים.
- **84 ‏hotspots — נדחה:** ריפקטור "בלי שינוי התנהגות" לקבצים גדולים
  שכולם ירוקים = ‏churn עם סיכון ובלי בעיה; ‏jscpd לא בריפו.
