...
> ---
>
> **QA 2026-08-07. המסמך הזה מסומן BINDING, והוא מתאר מערכת אחרת מזו שנשלחה.**
> הוא נשאר, כי מחקר ה-v11 שבו הוא הנכס היחיד שיש ליום שבו נעבור. אבל מי
> שקורא אותו כמפרט מימוש היום יכתוב קוד שאינו מתחבר לכלום. שלושה פערים,
> מדודים מול הקוד ומול הפרודקשן:
>
> **(א) ה-API. המסמך: v11 JSON REST. הקוד: legacy form-urlencoded.** במסמך
> מופיעים 17 אזכורים של `/Transactions/`, `/Documents/` ו-`/LowProfile/`,
> ואפס אזכורים של `/Interface/`. ‏`src/lib/payments/cardcom.ts` קורא **רק**
> ל-`/Interface/LowProfile.aspx`, ‏`/Interface/ChargeToken.aspx`,
> ‏`/Interface/GetLpResult.aspx` ו-`/Interface/RefundDeal.aspx`. הכרעה מ-23.07:
> הקוד נשאר legacy. הקוד עצמו נושא את האזהרה - `cardcom.ts:149` מחזיק את
...
> `src/app/api/payments/cardcom/webhook/route.ts`, וההגנה היא `?s=<secret>`
> בלתי-ניחוש ב-IndicatorUrl ולא path סודי מאחורי WAF.
>
> **מה שנבדק ונמצא נכון, ומיושם ככתבו:** סעיף 5 סעיף 1. הקביעה
> "‏ה-webhook של Cardcom אינו חתום (אין HMAC)" נכונה, והקוד מקיים אותה -
> אין המצאת חתימה, ויש אימות חוזר שרת-לשרת ב-`GetLpResult` כמקור האמת היחיד.
> זו הקביעה החשובה ביותר במסמך והיא עמדה במבחן.
>
> ראה `GAPS-CODE-VS-DOCS.md` ‏G8.
...
2. **הגנת URL.** ‏**המימוש בפועל שונה מהמתוכנן כאן, והכוונה זהה:** ה-endpoint
   הוא route handler של Next.js ב-`src/app/api/payments/cardcom/webhook/route.ts`,
   ולא Cloudflare Worker. הסוד אינו ב-path אלא ב-query: ‏`?s=<secret>`
   בלתי-ניחוש שנשלח ב-IndicatorUrl ונבדק בכניסה. אין WAF. **מה שנושא את
   הביטחון הוא סעיף 1 של הרשימה הזו** - האימות החוזר ב-`GetLpResult` - ולא
   הסוד ב-URL, שהוא סינון ראשוני בלבד.
