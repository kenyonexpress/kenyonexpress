...
**קודם כל: לוודא שהתהליך החשוד אינו אתה עצמך.** ‏`kenyon-loop.sh` מריץ
`claude --dangerously-skip-permissions --continue -p "<שורת התור>"`, ולכן שורת
הפקודה של הסוכן היא הפרומפט שלך **מעצם הבנייה** — התאמה מדויקת של הפרומפט אינה
ראיה לסוכן שני. ההבחנה היא שושלת האבות:

```bash
P=$PPID; while [ -n "$P" ] && [ "$P" != 1 ]; do
  ps -o pid,ppid,command= -p $P | tail -1 | cut -c1-90
  P=$(ps -o ppid= -p $P | tr -d ' ')
done
```

אם ה-PID החשוד מופיע בשרשרת — **זה אתה**, ואין תחרות. רק תהליך `claude` שאינו
אב הוא סוכן שני. שים לב גם ש-`grep -c 'claude --dangerously'` מנפח את הספירה,
כי שורת לולאת ה-`bash` מכילה את המחרוזת בלי להיות סוכן.

זה לא תיאורטי: ב-08.09.2026 עשרים סבבי תחזוקה דחו `pnpm build` על סמך זיהוי
שגוי כזה. הבנייה, כשרצה לבסוף, ארכה ארבע דקות והחזירה 0. **דחיית עבודה אמיתית
על זיהוי שגוי אינה הצד הבטוח של ההחלטה.**
