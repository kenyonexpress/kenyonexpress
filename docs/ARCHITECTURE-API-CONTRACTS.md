...
> ⛔ **‏מיושן החל מ-01.09.2026. המסמך המחייב הוא הקוד עצמו, תחת `src/app/api/`.**
>
> ‏**‏G-7 בטבלת הפערים שגוי לחלוטין:** "אין route handlers כלל" נכתב כשהתיקייה
> הייתה ריקה. היום יש **‏30** קבצי `route.ts`, כולל עשרת ה-crons, ה-webhook של
> ‏Cardcom ו-`/api/health`.
>
> שאר שורות ה-G לא נבדקו אחת-אחת מול הקוד בתאריך הזה, ולכן אין לקרוא את הטבלה
> כרשימת עבודה. החוזים והאינוריאנטים שבשאר המסמך עדיין תקפים.
