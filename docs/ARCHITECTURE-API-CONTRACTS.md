...
> ⚠️ **‏טבלת הפערים (סעיף ‏7, ‏G-1 עד ‏G-8) מיושנת. סומן 01.09.2026.**
>
> ‏**‏G-7 שגוי לחלוטין:** ‏"אין route handlers כלל" נכתב כשהתיקייה הייתה ריקה.
> ‏היום יש **‏30** קבצי `route.ts` תחת `src/app/api/`, כולל עשרת ה-crons,
> ‏webhook של Cardcom ו-`/api/health`. שאר שורות ה-G נסגרו חלקן ולא נבדקו
> אחת-אחת מול הקוד בתאריך הזה, ולכן אין לקרוא את הטבלה כרשימת עבודה.
>
> החוזים והאינוריאנטים שבשאר המסמך עדיין תקפים.
