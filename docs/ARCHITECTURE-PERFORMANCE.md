...
> **איחוד 2026-07-20:** מקור האמת המאוחד לביצועים + SEO בזמן ריצה הוא
> `ARCHITECTURE-PERFORMANCE-SEO.md` (שורש הפרויקט). המסמך הזה נשאר כרקע
> והיסטוריית הכרעות D-1..D-12 + טיוטת 038. בכל סתירה: המסמך בשורש גובר
> (כולל PS-9: Lighthouse = אזהרה ב-nightly לפי TESTING-CICD D8/D26, לא
> חסימת PR).

**סטטוס:** נבלע / רקע. כל סעיף כאן היה החלטה סגורה ב-2026-07-17.
**תאריך:** 2026-07-17 (pointer עודכן 2026-07-20)
...
**הערת בעלות:** שינויי קוד ומיגרציות המפורטים כאן מיושמים על ידי בעלי
`src/` ו-`supabase/` בהתאמה. SQL מוצע כאן מסומן כטיוטה למיגרציה 038.
