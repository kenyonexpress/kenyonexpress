...
> **צינור ביצוע קנוני (2026-08-11):** `docs/PAYOUT-ARCHITECTURE.md`  
> (`TransferFromDigitalBank` + `payout_statements`). מסמך זה נשאר BINDING לפירוט באצ', זכאות, ו-CSV fallback.
...
`settlement_events`
כמקור סכום. טבלת הריצה הקנונית:
`payout_statements`
(+ `supplier_bank_accounts`) לפי
...
**צינור ביצוע קנוני:** Cardcom `TransferFromDigitalBank` אחרי אישור אדמין, לפי
`PAYOUT-ARCHITECTURE.md`
(T+3, מינימום ₪100, `supplier_debit`, reconcile יומי).

**Fallback עד שה-Financial מופעל מסחרית / כש-Transfer נכשל:** ייצוא CSV + העברה בנקאית ידנית מתוך באצ' מאושר (מסה"ב / העברות מרובות) מחוץ למערכת, עם הזנת אסמכתה.

סליקת הלקוח נשארת Low Profile / legacy Interface לפי הקוד החי; payout לספק הוא שכבת v11 Financial נפרדת.
...
| PY4 | ביצוע כסף קנוני: Cardcom `TransferFromDigitalBank` (ראה PAYOUT-ARCHITECTURE); CSV = fallback; אישור אדמין לפני Transfer. |
...
| 2026-08-12 | batch-2 #20 pass-2: BINDING על arch/docs-batch-2 (המשך תור) |
