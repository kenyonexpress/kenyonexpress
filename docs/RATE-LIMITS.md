...
אחד. שתי השורות האלה הן המקומות היחידים שבהם פעולה של אדמין נמנית, והן שם
בכוונה: זו הפעולה היחידה באדמין ששורפת שובר של לקוח.
...
## ‏9. השכבה המדורגת מעל ‏`/api/*` ‏(‏`src/lib/rate-limit/graduated.ts`)

הטבלה בסעיף 2 נותנת לכל ראוט מספר אחד על חלון אחד. השכבה המדורגת יושבת
**לפניה**, ב-`proxy.ts`, לפי כתובת הלקוח ולפני רענון הסשן, כך שבקשה שנדחית
לא עולה סיבוב ל-Supabase. שלושה חלונות לכל כתובת, מהצר לרחב, וקירור
שמכפיל את עצמו בכל דחייה. **‏Upstash בלבד, בלי נפילה ל-Postgres:** זה רץ
על כל בקשת ‏API, וסיבוב ‏Postgres על כל אחת בזמן תקלת ‏Redis הוא תקלה
שנייה. לא מוגדר או למטה: ההחלטה פתוחה ונרשמת ב-error, והרצפה של סעיף 2
ממשיכה להחזיק. (השורות כאן אינן חלק מטבלת הסחיפה של סעיף 2; המקור הוא
‏`GRADUATED_POLICIES` והטסט ‏`graduated.test.ts`.)

| מדיניות | burst | sustained | daily | קירור |
| --- | --- | --- | --- | --- |
| api-anon | 30 / 10 s | 300 / 5 min | 5000 / day | 10 s, מוכפל בכל דחייה, תקרה 15 min, נשכח אחרי 1 h |
| api-device | 60 / 10 s | 900 / 5 min | 20000 / day | 5 s, מוכפל, תקרה 5 min, נשכח אחרי 15 min |

מי לא נמדד שם (‏`edge-shield.ts`): ‏cron, ‏jobs, ‏webhooks, ה-callback של
‏Cardcom, ה-workers של ‏QStash וה-uptime relay. כולם מוכיחים את עצמם בסוד,
וכולם מגיעים מכמה כתובות משותפות. גוף ה-429 נוקב ב-`refused_by`
‏(‏burst / sustained / daily / penalty) וב-`retry_after_seconds`.
