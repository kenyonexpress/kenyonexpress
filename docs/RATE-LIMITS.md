...
| `app-consent` | 20 | 1 h | in-app consent changes, per user |
...
## ‏9. ‏10.09: דחייה הייתה הדבר האחד שהשכבה הזאת לא רשמה

**כל שורת לוג ב-`src/lib/rate-limit/` עסקה בכך שהמגביל עצמו נכשל** —
‏`rate_limit.upstash_failed`, ‏`rate_limit.upstash_unreadable`,
‏`rate_limit.admin_client_unavailable`, ‏`rate_limit.open`. אף אחת מהן לא נורית
כשקורא **נדחה** בפועל.

לכן "כמה לקוחות נגעו בתקרת הקאשר היום" לא הייתה שאלה עם תשובה, **ותקרה שהודקה
יותר מדי הייתה בלתי נראית עד שמישהו מתקשר**: ‏`phone-otp` מתיר ‏5 לשעה לפי ‏IP
ו-`begin_checkout` ‏10 לדקה, ומשרד מאחורי כתובת אחת או קונה שמנסה שוב אחרי
כרטיס שנדחה מגיעים לשתיהן בלי לעשות שום דבר לא בסדר. הסעיף ביקש "ניטור פגיעות
בתקרה", וזה מה שלא היה.

‏`rate_limit.rejected` נורית עכשיו ב-`rateLimitByKey`, שהוא המקום היחיד ששני
ה-backends עוברים בו — **ולא באתרי הקריאה**, כי יש שלושים מהם ודחייה בלתי נראית
היא בדיוק מה שקורה כשכל אחד מהם צריך לזכור לרשום אותה בעצמו.

**שורה אחת לכל מפתח לכל חלון, ולא לכל בקשה.** ברגע שקורא חרג מהתקרה גם כל בקשה
הבאה נדחית, ולכן רישום כל אחת מהן הופך בוט למבול לוג שקובר את האות שבשבילו הוא
נוסף. ‏`shouldReportRejection` מדווח על הדחייה **הראשונה** בחלון ושותק בשארו.
המצב הוא ‏Map בתהליך, כמו ה-breaker של ‏Meilisearch, ולכן כל מכונה חמה מדווחת
את הראשונה שלה — **וזה מספיק לשאלה שנשאלת** (אילו תקרות נוגעים בהן בכלל) ואינו
מונה. המונה הוא ‏`RateLimit-Remaining`, והוא הולך לקורא.

**המזהה מגובב, תמיד.** מפתח רייט-לימיט נושא את מה שהקורא מוגבל לפיו: עבור
‏`phone-otp-number` זה **מספר טלפון** ועבור מדיניות ה-IP זו **כתובת IP**. שניהם
מידע אישי, **והמנקה שמגן על הלוגים עובד על שמות שדות** — ולכן מספר טלפון בתוך
מחרוזת שנקראת ‏`key` היה עובר אותו בלי לעצור. בשורה נרשם שם המדיניות בגלוי,
ו-12 תווי ‏SHA-256 של המזהה: מספיק לראות שקורא אחד אחראי לאלף דחיות, וחסר תועלת
כדי ללמוד מי הוא.

‏`src/lib/rate-limit/rejections.test.ts` מחזיק את שני הדברים: שהמזהה אינו יוצא
בגלוי, ושחלון שני מדווח מחדש בזמן שאותו חלון אינו.
