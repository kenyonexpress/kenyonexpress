...
| 5 | medium | Ofir (MCP) | 33 migrations sit in `migrations/pending/` unapplied (re-counted 2026-09-09; four that were filed there turned out to be applied already and were moved, and three were added since) — five of them are what turns payment_events, refunds destination, analytics, payouts and ai_usage fully on | Applying is a hard stop reserved for Ofir via MCP, in APPLY-ORDER.md order. Each of this closeout's six carries a rolled-back production dry run. |
