...
| `wallet_balances` | 006 | קיימת, 0 שורות | **DEPRECATED**. ‏~~לא נקראת ולא נכתבת~~ — **נקראה עד ‏10.09**, ראה סעיף ‏9. |
| `wallet_transactions` | 006 / 026 | קיימת, 0 שורות | **DEPRECATED**. גם היא נקראה עד ‏10.09. |
...
## 9. ‏10.09: מי כן קרא את הטבלאות ה"נטושות", ומה עוד לא נצפה

**סעיף ‏1 אמר על ‏`wallet_balances` "לא נקראת ולא נכתבת". זה לא היה נכון,
והקורא היה מסך אדמין.**

‏`/admin/users/[id]` — מסך ה-360 של הלקוח, זה שהתמיכה פותחת כשלקוח מתקשר —
קרא את ‏`wallet_balances` ליתרה ואת ‏`wallet_transactions` לתנועות. שתיהן
מחזיקות **אפס שורות** בפרודקשן. נמדד ‏10.09:

| מה | הערך בפרודקשן |
|---|---|
| ‏`wallet_accounts` | ‏13 שורות; לקוח אחד עם ‏₪1.80 |
| ‏`wallet_entries` | ‏2 שורות, שתיהן ‏`order_cashback` של ‏90 אגורות |
| ‏`wallet_balances` | ‏0 |
| ‏`wallet_transactions` | ‏0 |

כלומר המסך הציג לאותו לקוח **‏₪0.00 ו"אין תנועות ארנק"**. לא שגיאה ולא מצב
ריק מסויג: מספר שגוי בביטחון מלא על כסף של מישהו, בדיוק במסך שנפתח כשאותו
מישהו מתקשר לשאול לאן הכסף הלך. **הדריפט תועד נכון ב-02.08 והקורא פשוט לא
נספר**, וזה מה שהופך שורת "לא נקראת" במסמך למסוכנת יותר מהיעדר שורה.

התיקון קורא את ‏`wallet_accounts` ואת ‏`v_wallet_ledger`, דרך **הלקוח
מוגבל-הבקשה ולא לקוח השירות**: ‏`wallet_accounts_owner_read` ו-
‏`wallet_entries_select_unified` שתיהן כוללות ‏`is_admin()`, והתצוגה היא
‏`security_invoker=true`, ולכן המסד הוא הסמכות ואי-אפשר להראות בטעות ארנק של
לקוח אחר. הסכומים המצטברים נגזרים מהיומן במקום להישמר בעמודה, ולכן אינם
יכולים לסטות ממנו.

### 9.1 שני דברים שנבנו ואיש לא הסתכל בהם

**‏`v_wallet_balance_drift` קיימת מ-142 ולה קורא אחד בלבד בריפו:**
‏`scripts/security-probe-views.mjs`, שבודק שהיא **אינה** קריאה אנונימית. אף
אחד מעולם לא הסתכל **במה שהיא אומרת**. ארנק שהיתרה השמורה שלו חלוקה על היומן
שלו הוא המצב הגרוע ביותר של התת-מערכת הזו, כי שני המספרים נראים סמכותיים
והלקוח רואה אחד מהם. היא מוצגת עכשיו ב-`/admin/cashback`, **וכישלון קריאה
נאמר כ"הבדיקה לא רצה" ולא כתעודת תקינות** — נמדד היום: ‏0 שורות, כלומר אין
סטייה.

**‏`fn_cashback_expire(p_limit)` קיימת בפרודקשן ואף אחד אינו קורא לה.** אפס
הפניות ב-`src/`, ב-`scripts/`, ב-`migrations/` וב-`supabase/`. היא סורקת
זיכויי קאשבק שגילם מעל **12 חודשים** ולא נוצלו, מחזירה אותם ל-
‏`platform:cashback_reserve` (חשבון שקיים, ‏`02148d50-...`, והוא הצד המחייב
של שני הזיכויים הקיימים), וכותבת שורת ‏`cashback_ledger` עם
‏`entry_type='expiry'`.

**היא לא חוברה לתזמון, וזו החלטה ולא השמטה.** הפעלתה **לוקחת כסף מלקוחות**
לפי כלל שאיש לא הודיע עליו: עמוד הארנק אומר "אין משיכה למזומן ואין העברה
למשתמש אחר" ואינו מזכיר שעון של ‏12 חודשים. תחת חוק הגנת הצרכן הישראלי אמירה
עובדתית בעמוד מחייבת את העסק, והיעדר אמירה על פקיעה אינו רשות לפקיעה שקטה.
בכיוון ההפוך, **להוסיף לעמוד "הקאשבק פג אחרי שנה" גם הוא המצאת מדיניות**,
כי כרגע שום דבר אינו מפקיע. שתי הדרכים הן החלטה עסקית, ולכן שתיהן לא ננקטו
והממצא נרשם.

**מכאן גם נובע שסעיף "תזכורת ‏T-1" של ‏SECTIONS 32 חסר תנאי מוקדם**: אין למה
להזכיר. אין עמודת פקיעה על אף טבלת ארנק או קאשבק (נבדק ב-`information_schema`),
ואין ‏job שמריץ את הפונקציה.

### 9.2 ‏`cashback_ledger` אינו הרישום המלא של הקאשבק ששולם

‏177 ממתינה, ולכן ‏`cashback_ledger` מחזיקה ‏0 שורות — בזמן שרזרבת הקאשבק
כבר שילמה שני זיכויים אמיתיים דרך ‏`wallet_entries`. קונסולה שהמשפט היחיד שלה
הוא "אין תנועות ביומן עדיין" נקראת כ"לא שולם קאשבק מעולם", וזה שקר היום
**ונשאר שקר אחרי ש-177 תוחל**, כי היא מתחילה לרשום מאותו רגע ואינה מייבאת
אחורה. המסך סופר עכשיו את שני המקורות בנפרד ואומר את ההבדל.

### 9.3 מה שנבדק ונמצא תקין

- **הפנקס באמת ‏append-only.** ‏`wallet_entries_append_only` ו-
  ‏`cashback_ledger_append_only` פעילים. הוכח ב-`DO` שהתגלגל אחורה:
  ‏`UPDATE` ו-`DELETE` שניהם נחסמו עם ‏`P0001 wallet_entries is append-only;
  post a compensating transfer instead`, ואף שורה לא נגעה.
- **"אין כפתור משיכה" ו"הודעת יתרה אפס"** — שניהם קיימים בעמוד
  ‏`/account/wallet`.
- **"‏job יומי שמזרים עודפי escrow לארנק"** — קיים תחת שם אחר:
  ‏`credit_expired_vouchers()` בתוך משימת ‏`expire-vouchers` היומית מזכה את
  ארנק הלקוח בכסף של שובר שפג. זה אותו מסלול כסף בדיוק.
