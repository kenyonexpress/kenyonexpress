...
## 5. אותות אמון, מנתונים אמיתיים בלבד (‏Q19, ‏25.09)

שני אותות ציבוריים, ושניהם נגזרים ממה שקרה ולא מדגל שמישהו סימן.
כשאין ראיה, אין אות: לא מספר מעוגל, לא "כמה", לא "מאומת" מהגדרת ברירת מחדל.

| אות | מה נחשב | מה לא נחשב | איפה |
| --- | --- | --- | --- |
| **‏`ספק מאומת`** בדף המוצר וב-`/s/[id]` | בקשת הצטרפות שאושרה על ידי אדם (‏`supplier_applications.status='approved'` עם ‏`supplier_id`, אחרי ‏204), **או** מימוש אמיתי אחד לפחות (‏`vouchers.status='redeemed'` אצל הספק). ספק חייב להיות ‏`active` ולא מחוק | ‏`suppliers.status='active'` לבדו (ברירת המחדל של העמודה מ-027, כל שורה זרועה נושאת אותה); ‏`business_id` (‏0 מ-7 פעילים) | ‏`lib/suppliers/verification.ts` (טהור), ‏`verification-read.ts` (שני ‏head counts) |
| **‏`N נרכשו השבוע`** מעל כפתור הקנייה | יחידות ב-`order_items` על הזמנות ששולמו ב-7 הימים האחרונים (‏`paid`/`fulfilled`/`partially_fulfilled`/`platform_settled`) **עם חיוב אמיתי**: ‏`payments.status='succeeded'`, ‏`kind='charge'`, ומזהה ספק שאינו ‏`mock-` | הזמנות ‏mock (נמדד ‏25.09: **‏18 מ-18** ההזמנות ששולמו בשבוע האחרון בפרודקשן הן ‏mock); ‏`refunded`/`cancelled`; פריט מבוטל; ספירה מתחת לרצפה ‏3 | ‏`lib/commerce/social-proof.ts` (הכלל והרצפה), ‏`bought-this-week.ts` (שתי קריאות), ‏`BoughtThisWeek.tsx` |

הרצפה ‏3 היא החלטת תצוגה ולא מספר: מתחתיה השורה נעדרת ולעולם לא מעוגלת. הקידומת
‏`mock-` היא קבוע אחד ב-`lib/payments/mock-transaction-id.ts` שגם ספק ה-mock קורא,
ולכן השניים לא יכולים להתפצל.
