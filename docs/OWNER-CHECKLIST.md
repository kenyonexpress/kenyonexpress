...
‏נכתב ‏01.09.2026, עודכן ‏02.09 בסוף שבעת מגה-הבלוקים (תגי `v1.4`–`v1.8`,
‏`v2.0.0-rc1`). זה המסמך היחיד שאתה צריך לקרוא.
...
| # | פריט | זמן | חוסם מה |
|---|---|---|---|
| ‏1 | ‏מיגרציות ‏147–157 דרך MCP לפי ‏APPLY-ORDER.md, ואז `pnpm db:types` | ‏30 דקות | ביקורות, משאלות, משלוחים, payouts, refunds — כולם "מושבתים בעדינות" עד אז |
| ‏2 | מסוף Cardcom לייצור (‏CARDCOM_USE_MOCK=false + ‏4 מפתחות) | שיחת טלפון + ‏10 דקות | אי אפשר לגבות שקל |
| ‏3 | ניתוק ה-DNS (הזון הניתן לעריכה **אינו** המגיש — ‏DNS-CUTOVER-PLAN.md) | ‏30 דקות + המתנה | האתר האמיתי לא באוויר |
| ‏4 | מיזוג ‏PR הריליס | ‏2 דקות | ‏`main` מתאר מוצר שלא קיים |

‏(המתזמן כבר חי ב-GitHub Actions מ-02.09 — **12 jobs**; ‏cron-job.org נשאר
אופציונלי בלבד. ‏Sentry Uptime על ‏/api/health חי גם הוא, ‏monitor 2159284.)
