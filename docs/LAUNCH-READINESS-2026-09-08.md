...
| 11 PAYMENTS | ⚠️ **corrected 2026-09-08** | State machine is `pending → paid → split_executed`; `authorized`/`captured` belong to the escrow model migration 085 removed. Cardcom sends no signature to verify. **Webhook idempotency was never assessed and is now the open item.** It is implemented correctly — the handler inserts into `payment_webhook_events` first and treats 23505 as "Cardcom delivered this twice" — but all of that correctness is borrowed from a UNIQUE constraint on `(provider, external_event_id)`, and **whether that constraint exists in production cannot be established from this checkout**: `supabase/migrations/` declares it and does not describe production, the generated types cannot express uniqueness, anon sees none of these tables, and the admin key answers 401. Without it a redelivered callback finalises an already-charged card a second time. `scripts/audit-money-constraints.mjs --sql` answers this and the three sibling defences in one paste. See MANUAL item 11. |
...
| 11 | **Run `node scripts/audit-money-constraints.mjs --sql` through the Supabase MCP tool and pipe the result back with `--from`** | Confirms the four UNIQUE constraints the money path relies on to make a replay a no-op — `payments.idempotency_key`, `ledger_journals.event_key`, `payment_webhook_events(provider, external_event_id)`, `wallet_entries.idempotency_key`. Read-only, one query. The webhook one is the urgent one: without it a redelivered Cardcom callback finalises a card that was already charged. |
