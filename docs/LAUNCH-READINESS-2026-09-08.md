...
| 9 | Set repo **variables** `RLS_SUPABASE_URL` and `RLS_SUPABASE_ANON_KEY` to this project's public URL and anon key | un-skips 12 of the 25 RLS tests in CI. Both values are already public - the anon key ships in every page's client bundle - so these are variables, not secrets. Does not touch `secrets.CI_SUPABASE_*`, which stays unset to keep the E2E seeding job disabled. |
