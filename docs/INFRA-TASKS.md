...
**סטטוס:** ✅ בוצע 22.09.2026, `scripts/verify-cutover.mjs`.

**מה בוצע, זהה לספק למטה עם שינוי אחד:** אותם שלושה מצבי יציאה, אותן
בדיקות, אותו resolver ‏(1.1.1.1). השוני היחיד: הבדיקה "‏TLS תקין"
במקור הייתה בדיקה נפרדת; בפועל `fetch` על `https://` שנכשל על תעודה
לא תקפה כבר נופל תחת "‏homeOk" ‏(הבקשה עצמה נכשלת), כך שאין בדיקת TLS
נפרדת אבל התוצאה מכוסה. **נמדד, לא רק נכתב:** הרצה חיה נגד
`kenyonexpress.co.il` היום מחזירה מצב ‏2 עם ההודעה "טרם הופנה" —
תואם בדיוק את המצב המדוד ‏(אין ‏NS, אין ‏A) לאורך כל הסשן הזה. משולב
ב-`production-smoke.yml` ‏(שלב `continue-on-error`, נכשל בפועל רק
כש-`vars.PRODUCTION_URL` כבר מוגדר ל-apex), ו-`docs/DNS-CUTOVER-PLAN.md`
מפנה אליו בסעיף האימות עם סעיף חדש "היום שאחרי" לצעדי הלוח הידניים
‏(‏DOM5, ‏DOM8, `NEXT_PUBLIC_APP_URL`, משתני ה-repo). ‏VCL6/VCL7 ‏(הצמדת
הדומיין בלוח Vercel) נשארו ידניים במוצהר — שום דבר בריפו יכול לבצע
אותם. טסטים: `verify-cutover.test.mjs` ‏8/8 ‏(`classify()` טהור, בלי
רשת).
...
**סטטוס:** ✅ בוצע 22.09.2026, `scripts/secret-leak-gate.mjs`.

**מה בוצע, זהה לספק:** סריקת `.next/static/` בלבד מול מועמדים שנגזרים
מ-`process.env` בזמן ריצה ‏(לא grep על קוד) וגם מול שבעת שמות ה-P0
‏(`SUPABASE_SECRET_KEY`, סט Cardcom, `CRON_SECRET`, `VOUCHER_QR_SECRET`
— שבעה ולא שישה, כי סט Cardcom הוא ארבעה משתנים נפרדים; מנוסח בקוד
עם ההסבר). שלושה מצבי יציאה, ערך לעולם לא מודפס. חוברה ל-`package.json`
‏(`gate:secret-leak`) ול-`ci.yml` מיד אחרי `pnpm build`, עם תיעוד כן
שהריצה ב-CI מוכיחה בעיקר מנגנון כי אין שם סוד אמיתי. הצ'קליסט
‏(ENV14, ‏S1, בלוק "בדיקת דליפה") מפנה לסקריפט. **נמדד**: הרצה חיה מול
ה-build האמיתי של הסשן הזה חוזרת נקייה. טסטים: `secret-leak-gate.test.mjs`
‏15/15 על fixtures אמיתיים ‏(תיקיית temp, בלי סוד אמיתי אף פעם).
