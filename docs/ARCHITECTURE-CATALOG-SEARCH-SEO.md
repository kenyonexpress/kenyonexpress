...
> **QA 2026-08-06: נמצאה כאן סתירה חזיתית מול מסמך שכבר עבר QA-PASS, והוכרעה.**
>
> סעיף 2.5 דירג לפי מרג'ין: `0.10 * margin` כאשר
> ‏`margin = coalesce(platform_percent, 10) / 100`. ‏`ARCHITECTURE-SEARCH-UX.md`
> (‏QA-PASS #4) קובע במפורש: "דירוג לא משתמש בעמלה קבועה; אין boost לפי
> ‏10%/5%. ‏`platform_percent` אינו גורם מיון ב-UX". שניהם לא יכולים לחיות.
>
> **ההכרעה: מסמך ה-UX מנצח, ואיבר המרג'ין הוסר.** שתי סיבות, ושתיהן עומדות
> בפני עצמן. ‏**(א)** ה-`coalesce(..., 10)` הוא בדיוק ברירת המחדל שבוטלה
> ב-C1/C2 ובמיגרציה 050, ובהקשר הזה הוא גרוע במיוחד: מוצר בלי אחוז לא היה
> נכשל אלא **מקבל דחיפה בדירוג**. ‏**(ב)** דירוג תוצאות לפי מה שמכניס יותר
> לפלטפורמה, בלי לומר זאת למשתמש, הוא חשיפה צרכנית ולא כוונון. עשירית מהציון
> מספיקה כדי להפוך סדר בין שני דילים קרובים.
>
> המשקל שוחרר לטובת רלוונטיות. שאר המסמך נבדק ואינו נושא את מודל הכסף הישן.
...
```
score = 0.65 * lexical    -- ts_rank_cd(search_vector, q, 32): רלוונטיות, משקולות A/B/C
      ...
      -- אין איבר margin. platform_percent אינו גורם מיון (SEARCH-UX, QA-PASS #4).
```

- רלוונטיות שולטת (0.65 + 0.15).
- ~~margin הוא tie-breaker מודע-עסק~~ **הוסר ב-QA של 06.08**, ראה את הבאנר
  למעלה. שובר-שוויון בין שני מוצרים רלוונטיים באותה מידה הוא `featured`, שהוא
  החלטה מפורשת של האדמין פר מוצר, ולא נגזרת שקטה של הרווח.
