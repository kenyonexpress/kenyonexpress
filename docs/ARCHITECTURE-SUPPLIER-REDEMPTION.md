...
לכל מוצר יש `platform_percent` שנקבע על ידי אדמין. **שדה חובה בלי ברירת מחדל**
(CONTRADICTIONS C1/C2, מיגרציה 050: `NOT NULL` בלי `DEFAULT`).

| סוג מוצר | הלקוח משלם באתר | הלקוח משלם בעסק | הפלטפורמה מרוויחה | הספק מקבל מהפלטפורמה |
|---|---|---|---|---|
| קופון (`coupon`) | `coupon_price` פר-מוצר (שדה חופשי, לא נגזרת אחוז; C4) | היתרה, במימוש | את כל התשלום באתר | 0 (הכסף שלו נגבה ישירות מהלקוח בעסק) |
...
ה-`held` על קופון הוא **רישום פנימי ב-ledger שלנו בלבד** עד המימוש: אין Escrow
חיצוני, אין צד שלישי נאמן ואין עסקת J5 מול Cardcom (C3). הכסף יושב בחשבון
הסליקה שלנו לאורך כל הדרך.

שרשרת ה-snapshot של האחוז - **אין שרשרת ואין fallback** (C1/C2):

```
products.platform_percent  (חובה, NOT NULL, בלי DEFAULT)
```

`suppliers.commission_percent` אינו ידית פיצול; הוא נשאר רק כערך הצעה בהסכם
הספק בזמן הקמת מוצר. גם `products.commission_percent` הוצא משימוש (050,
קריאה בלבד לסנפשוטים שנוצרו לפני 050).
...
שינוי אחוז עתידי לא משנה דוחות עבר (C10). פונקציית עזר לחישוב בזמן checkout:
`public.product_platform_percent(product_id)` - מאז 050 היא מחזירה את ערך המוצר
כמות שהוא, בלי fallback ובלי הליטרל 10 שהיה בנוסח 027.

**תנאי תשלום לספק (C8, מיגרציה 051)**: שורה נכנסת ל-payout רק אחרי **T+3 ימי
עסקים** (ראשון-חמישי) מרגע האירוע הכספי (מסירה / מימוש), ודוח משולם רק כשהיתרה
הצוברת מגיעה ל-**מינימום 100 ש"ח**. מתחת לסף הדוח נסגר כ-`cancelled` עם
`rolled_over = true` והשורות מתגלגלות לריצה הבאה.
...
## 9. הכרעות המשך ושאלות פתוחות

1. **איחוד `vendors` -> `suppliers` (סגור):** `suppliers` קנונית; האיחוד מתוכנן
   ב-`036_vendors_unification.sql` לפני בניית פורטל הספקים (MASTER 1.12/2.9).
2. **הצפנת פרטי בנק (פתוח):** כרגע RLS + audit מסתיר. אם רגולציה או ביטוח
   ידרשו, תתווסף הצפנה ייעודית. אין שינוי עד דרישה מוכחת.
3. **תוקף קופון (סגור משפטית):** רצפת תוקף של 4 חודשים ממועד הרכישה.
   ולידציית השרת חוסמת תוקף קצר יותר
   (`ARCHITECTURE-LEGAL-COMPLIANCE.md` סעיף 1.2).
4. **קופון שפג בלי מימוש (סגור משפטית):** job הפקיעה מזכה אוטומטית את ארנק
   הלקוח במלוא `platform_paid_ils` כ-`refund_credit`, בתוקף 5 שנים. אין
   הכרת breakage בסכום ששולם לפני הזיכוי.
5. **מחיר קופון (סגור):** `platform_price` הוא מחיר קופון חופשי שנקבע פר דיל,
   לא נגזרת של `platform_percent` (MASTER 1.40).
6. **ריבוי ספקים למשתמש ב-UI (פתוח):** המבנה תומך,
   `current_supplier_id()` בוחר את הראשון. אם יידרש מיתוג ספקים, נוסף בורר
   ומועבר `supplier_id` מפורש ל-RPCs.
7. **זהות עסקת Cardcom (סגור):** המקור הקנוני הוא
   `payments.cardcom_transaction_id` עם UNIQUE; השדה
   `orders.cardcom_payment_id` נשאר write-through בלבד (MASTER 1.10).
