...
## עדכון ‏07.09.2026: ‏W1 בלוקים 9 עד 13

**כל מה שמתחת לקו הזה נכתב ב-01.09 ומסומן היסטורי.** הסעיף הזה הוא המצב
הנוכחי, והוא מתקן שלושה דברים שהמסמך הישן אומר.

### מה נסגר

| בלוק | מה נמצא |
|---|---|
| 9 ‏Observability | ‏Sentry ו-Axiom היו בנויים. **‏PostHog לא ראה אף המרה מעולם**: שני האירועים שקובעים לא יכולים להגיע מהדפדפן, כי `purchase` לא נורה מהלקוח במכוון ו-`voucher_redeemed` קורה על קופה של ספק |
| 10 ‏E2E | הטסט "הכושל" לא היה כושל. מה שחסר היה כיסוי לארנק, ל-CRUD אדמין ולגבולות התפקידים |
| 11 ‏SEO | ‏`LocalBusiness` לא היה קיים. ‏`/products` התחרה בעצמו בלי canonical. שלושה ‏URL של checkout היו ניתנים לאינדוקס, ואחד מהם נושא `order_id` |
| 12 נגישות | הפלטה תקינה. **מחוון הפוקוס ב-checkout היה 1.41:1** מול רצפה של 3:1 |
| 13 ‏CI | שער הפיקסלים לא רץ בשום מקום מלבד לפטופ. ‏migration lint לא בדק **היכן** קובץ נוחת |

### שלושה תיקונים למסמך ההיסטורי

1. **"הקוד גמור ואין מה לכתוב"** לא היה נכון. חמישה בלוקים ייצרו ‏13 commits
   של קוד, מהם ארבעה פגמי כסף או אבטחה אמיתיים.
2. **מספר הבלוקים האנושיים אינו ארבעה אלא שמונה.** נוספו: רוטציית
   ‏`SUPABASE_SECRET_KEY`, שלושת ה-secrets של ‏CI, שלוש טבלאות ההסכמה
   המשפטית, ומיגרציה 176. הרשימה המחייבת היא `LAUNCH-CHECKLIST.md` סעיף 9א.
3. **"שורת הבדיקה נמכרת בשקל" מוגזם.** נמדד: היא חיה (‏`active`, ‏₪1 מתוך
   ₪400, ‏10 במלאי) **ואי אפשר לקנות אותה**, כי `MAX_PLAUSIBLE_DISCOUNT_PERCENT`
   הוא 95 ו-99.75% נדחה בשלוש נקודות. פגם תצוגה, לא קופה פתוחה.

### ארבעת הפגמים האמיתיים שנמצאו ותוקנו

1. **זיכוי לארנק החזיר כסף והשאיר קופון סָרִיק.** הכפתור אינו מגודר במכוון,
   ‏`refundOrderToWallet` לא הסתכל על שובר, והלקוח קיבל את כספו ושמר קופון
   שבית העסק יכבד. ‏`70605bf69`.
2. **‏PostHog בלי אף אירוע המרה**, ועם מלכודת זהות: שני חצאי המשפך החזיקו
   כל אחד מזהה שהשני לא רואה. ‏`a43b6f9e2`.
3. **מחוון הפוקוס ב-checkout בלתי נראה.** ‏`outline: none` עם גבול `#fed700`
   על שדה לבן. ‏`dd60ac508`.
4. **‏`verify_supplier_staff_pin` מאפסת את מונה הניסיונות בהצלחה**, והמפתח
   פר-קורא, כך שמי שמכיר ‏PIN אחד מקבל ניחושים בלתי מוגבלים. ‏176, לא הוחלה.

### מה שנמדד ולא היה פגם

‏`redeem_voucher` החי תואם בדיוק למודל בטסט החוזה. אפס טבלאות בלי ‏RLS,
אפס טבלאות עם ‏RLS ובלי מדיניות, ‏74 פונקציות ‏`SECURITY DEFINER` וכולן עם
‏`search_path` מוצמד. הפלטה עוברת ‏AA. שער הפיקסלים ‏10.68 / 7.72 / 8.12.
