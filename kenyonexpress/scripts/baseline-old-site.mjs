...
// ---------- 1. Lighthouse (mobile) ----------
{
  ...
  try {
    execSync(
      `npx -y lighthouse "${SITE}/" --output=json --output-path="${jsonPath}" --form-factor=mobile --screenEmulation.mobile --quiet --chrome-flags="--headless=new"`,
  ...
  if (existsSync(jsonPath)) {
    ...
    const audit = (id) =>
      r.audits[id] ? r.audits[id].displayValue || r.audits[id].numericValue : 'n/a'
    save(
      'baseline-old-site-performance.md',
      [
...
async function fetchText(url) {
  const res = await fetch(url, {
    headers: { 'User-Agent': 'Mozilla/5.0 (baseline-audit)' },
    redirect: 'follow',
  })
...
let allUrls = []
{
  const candidates = [`${SITE}/sitemap_index.xml`, `${SITE}/sitemap.xml`, `${SITE}/wp-sitemap.xml`]
  let rootXml = null
  let rootUrl = null
  for (const c of candidates) {
    try {
      rootXml = await fetchText(c)
      rootUrl = c
      break
    } catch {}
  }
  if (rootXml) {
    ...
    if (isIndex) {
      for (const sm of first) {
        try {
          allUrls.push(...locs(await fetchText(sm)).filter((u) => !u.endsWith('.xml')))
        } catch (e) {
          console.error('  sub-sitemap failed:', e.message)
        }
      }
    } else {
      allUrls = first
    ...
    const L = [
      `# Baseline: old site pages (${today})`,
      '',
      `Sitemap root: ${rootUrl}`,
...
// ---------- 3. title + description of main pages ----------
{
  ...
  for (const u of main) {
    try {
      ...
      rows.push({ u, title: `FETCH FAILED: ${e.message}`, desc: '', ogTitle: '' })
    }
  }
  const L = [
    `# Baseline: old site SEO (${today})`,
    '',
    '| URL | title | meta description | og:title |',
    '|---|---|---|---|',
  ]
  for (const r of rows)
    L.push(
      `| ${r.u} | ${r.title.replace(/\|/g, '\\|')} | ${r.desc.replace(/\|/g, '\\|')} | ${r.ogTitle.replace(/\|/g, '\\|')} |`,
    )
