...
const FILES = [
  ...
  [
    /baligam\.|groo\.|walla.*coupon|coupon.*\.co\.il/i,
    'competitor-israeli-coupons.md',
    'Israeli coupon sites: flow to checkout',
  ],
]
const fileFor = (url) => {
...
const EXTRACT = () => {
  ...
  for (const el of document.querySelectorAll(
    'span, div, p, del, s, ins, [class*="price"], [class*="discount"], [class*="save"]',
  )) {
  ...
  for (const el of document.querySelectorAll(
    '[class*="timer"], [class*="countdown"], [class*="count-down"], [class*="expir"], [class*="urgency"], time',
  )) {
    if (!vis(el)) continue
    const t = txt(el)
    if (t)
  ...
  for (const el of document.querySelectorAll(
    'button, a[class*="btn"], a[class*="buy"], input[type="submit"], [class*="cta"], [class*="add-to-cart"], [class*="checkout"]',
  )) {
  ...
  for (const el of document.querySelectorAll(
    '[class*="fine-print"], [class*="finePrint"], [class*="terms"], [class*="conditions"], [class*="tnc"], details',
  )) {
    if (!vis(el)) continue
    const t = txt(el)
...
async function capture(page) {
  ...
  for (const h of data.aboveFold)
    L.push(`- ${h.tag} ${h.aboveFold ? '[ABOVE FOLD] ' : ''}"${h.text}" @ ${h.pos}`)
  L.push('', '### Prices / discounts', '')
  for (const p of data.prices)
    L.push(
      `- "${p.text}" @ ${p.pos} ${p.struck ? '[STRIKETHROUGH old price] ' : ''}(${p.size} ${p.weight} ${p.color})`,
    )
  ...
  for (const c of data.ctas)
    L.push(
      `- ${c.aboveFold ? '[ABOVE FOLD] ' : ''}"${c.text}" @ ${c.pos} bg ${c.bg}, text ${c.color} ${c.size}, radius ${c.radius}`,
    )
  L.push('', '### Forms', '')
...
await first
  .goto('https://www.groupon.com/', { waitUntil: 'domcontentloaded', timeout: 60000 })
  .catch(() => {})

console.error(
  'Browse normally: Groupon deal page, then baligam.co.il and groo.co.il flows up to checkout.',
)
