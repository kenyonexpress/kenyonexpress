...
function md(title, rows, extra = '') {
  const lines = [
    `# ${title}`,
    '',
    `Source: ${BASE}, measured ${new Date().toISOString().slice(0, 10)}, headed Chrome computed styles.`,
    '',
  ]
...
// ---------- 1+2. cart + checkout ----------
{
  ...
  try {
    ...
    console.error('  add-to-cart failed, cart may be empty')
  }

  await open(desktop, `${BASE}/cart/`)
  await desktop.waitForTimeout(2000)
  save(
    'measurements-cart-page.md',
    md(
      'Cart page (1440px)',
      await dumpMany(desktop, [
...
let productUrl = null
{
  ...
  try {
    ...
    modalRows.push(['quick view', null])
  }
...

{
  ...
  await desktop.waitForTimeout(2500)
  save(
    'measurements-product-gallery.md',
    md(
      `Product gallery (${productUrl || 'product page'}, 1440px)`,
      await dumpMany(desktop, [
  ...
  for (const [label, sel] of [
    ...
    [
      'newsletter submit',
      '.footer-newsletter button, .newsletter form button, input[type="submit"]',
    ],
  ...
  save(
    'measurements-buttons.md',
    md(
      'Buttons, all variants (1440px)',
      btnRows,
  ...
  const spacing = await desktop.evaluate(() => {
    ...
    for (let i = 0; i < blocks.length; i++) {
      ...
      const r = el.getBoundingClientRect()
      const name =
        `${el.tagName}.${String(el.className).trim().split(/\s+/).slice(0, 2).join('.')}`.toLowerCase()
  ...
  const spLines = [
    '# Section spacing (home-v7, 1440px)',
    '',
    '| section | height | margin t/b | padding t/b | visual gap to next |',
    '|---|---|---|---|---|',
  ]
  for (const s of spacing)
    spLines.push(
      `| ${s.section} | ${s.height}px | ${s.marginTop} / ${s.marginBottom} | ${s.paddingTop} / ${s.paddingBottom} | ${s.gapToNext === null ? '' : `${s.gapToNext}px`} |`,
    )
...
// ---------- 5. forms: default, focus, error ----------
{
  ...
  try {
    ...
    formRows.push(['input :focus', null])
  }
  try {
    ...
    formRows.push(['error state', null])
  }
  save(
    'measurements-forms.md',
    md(
      'Forms: default / focus / error (my-account, 1440px)',
      formRows,
  ...
  // ---------- 9. account pages ----------
  save(
    'measurements-account-pages.md',
    md(
      'Account pages (my-account, logged out, 1440px)',
      await dumpMany(desktop, [
