...
|---|---|---|
| 040 money numeric → integer agorot (backfill, verify, CHECK) | `051_money_integer_units.sql` | add/backfill/verify/rename every `*_ils` → `*_agorot` |
| 041 percentage → basis points | `051_money_integer_units.sql` (same file) | every `*_percent` → `*_bp` (×100) |
| 042 coupon single-use race-safe | `053_coupon_single_use.sql` | CAS + partial unique + transition guard |
| 043 settlement_batches + items | `054_settlement_batches.sql` | per-supplier batch + item snapshot from order_items |
| 044 reconciliation_runs + discrepancies | `055_reconciliation.sql` | scheduled-integrity run + discrepancy tables |
| 045 ledger (accounts, journals, lines, RLS) | `050_ledger_core.sql` (+ RLS in `056`) | double-entry core with sum-zero + immutability |
| 046 idempotency_keys | `052_idempotency_keys.sql` | generic `(scope, key)` store |
...
| 049 RLS complete | `056_money_rls.sql` | RLS on every new money table |
...
### 3.1 `050_ledger_core.sql` — ledger (plan 045)
...
### 3.2 `051_money_integer_units.sql` — agorot + basis points (plan 040+041)
...
### 3.3 `052_idempotency_keys.sql` — generic idempotency (plan 046)
...
### 3.4 `053_coupon_single_use.sql` — race-safe redemption (plan 042)
...
### 3.5 `054_settlement_batches.sql` — per-supplier settlement (plan 043)
...
### 3.6 `055_reconciliation.sql` — integrity job tables (plan 044)
...
### 3.7 `056_money_rls.sql` — RLS complete (plan 049)
