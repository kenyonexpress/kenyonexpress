...
האדמין קובע עמלה **פר קמפיין** (‏`affiliate_campaigns`: ‏`commission_bp`
בנקודות בסיס ‏0..5000, מינימום הזמנה, תקרה למכירה, תקציב כולל, מכסה יומית
לשותף, אישור ידני, חלון, היקף לקטגוריה או למוצר), והזמנה ששולמה עם
‏`orders.affiliate_code` (עמודה מ-010 שאיש לא כתב אליה; הקופה כותבת עכשיו
מהעוגייה ‏`ke_ref`) נרשמת כ-`affiliate_conversions` (שורה אחת להזמנה, ‏UNIQUE)
ומזוכה לארנק השותף דרך ‏`fn_wallet_transfer` מ-`platform:cashback_reserve`
(אותו חשבון ש-`fn_pay_referral` מחייב), ‏idempotency ‏`affiliate:<id>`.
**ההחלטה כולה ב-TypeScript** (‏`src/lib/affiliates/commission.ts`, טהור,
דרך ‏`applyBp` ב-`money.ts`), לא ב-plpgsql, כי אחוז מכסף עובר רק דרך המודול
הזה. בדיקות הונאה: רכישה עצמית ובונוס חבר-מביא-חבר על אותה הזמנה נדחים
(שורה ‏`rejected`), אותו מכשיר/‏IP/כרטיס (‏`fn_referral_fraud_signals` מ-098,
מוזן עכשיו גם בהצטרפות ובקופה), מהירות מעל המכסה היומית ואישור ידני מנתבים
ל-`flagged` לתור ב-`/admin/affiliates`. ‏RLS: ‏SELECT לאדמין על הקמפיינים,
‏SELECT לשותף על השורות שלו ולאדמין על הכול בהמרות; אף כתיבה לאף תפקיד לקוח.
**הקוד רץ בלעדיה:** כל קורא תופס ‏42P01: דף החשבון אומר "התוכנית עדיין לא
פתוחה" ומאפשר להצטרף (‏`affiliates` מ-010 קיימת), הקישורים נושאים את הקוד,
הקופה מצלמת אותו, ו-finalize רושם ‏`affiliates.campaigns_table_missing`
ומשלם דבר. לא הורצה ‏BEGIN/ROLLBACK מול פרודקשן בסשן הזה.
...
מבוסס "המחיר הרגיל" המחוק (‏`products.original_price_source`, תווית קצרה,
ו-`original_price_source_url`, קישור ‏https אופציונלי לאסמכתא) ומקשר לדף
הביקורות של העסק בגוגל (‏`suppliers.google_reviews_url`, מוגבל ב-CHECK
למארחים של גוגל בלבד). שלוש עמודות ‏NULLable, ‏`ADD COLUMN IF NOT EXISTS`,
בלי שינוי בשום עמודה קיימת. ‏GRANT SELECT ברמת עמודה ל-`anon`
ו-`authenticated` על שתי עמודות המוצר, כי החנות קוראת ‏`products` במפתח
ה-anon; ‏`suppliers` נקראת דרך ה-service client ולא נוסף לה grant. **הקוד רץ
בלעדיה:** שתי הקריאות עוברות דרך ‏`readOptionalColumns` (‏42703 נרשם פעם
אחת ונקרא כ-NULL), כך שהדף מציג בלי מקור ובלי קישור עד ההחלה. המקור לא
מכריע אם המחיר המחוק מוצג בכלל, זה נשאר בידי פסק ‏30 הימים (‏193).
לא הורצה ‏BEGIN/ROLLBACK מול פרודקשן בסשן הזה.

## 2026-09-25: 241 WRITTEN, not applied - עיר על שלושה מוצרים ששמם נוקב בה, נתונים בלבד

‏`241_seed_product_city_from_title.sql`. ‏Q03: כרטיס הדיל בעמוד הבית מציג
עיר בשורת המטא (`products.city`, ואם ריק `suppliers.city`). נמדד מול
פרודקשן ב-25.09 עם מפתח ה-anon: ‏46 שורות פעילות, ‏`city` ריק בכולן, ואף מוצר
פעיל אינו שייך לספק עם עיר. הקובץ מציב עיר **רק** בשלושת המוצרים ששמם עצמם
נוקב בעיר (פתח תקווה, תל אביב, טבריה), בכתיב ש-`src/lib/geo/cities.ts` מכיר.
כל ‏UPDATE מסונן על ‏slug וגם על ‏`city IS NULL`, כך שערך שמפעיל הציב מאז לא
נדרס והחלה חוזרת היא ‏no-op. אין שינוי סכימה. הקוד עובד בלעדיו: השורה פשוט
לא מציגה עיר.
...
התשלומים היומית ‏(‏`/api/cron/payout-run`) הכינה לו דוח. **הקובץ קורא את
רשימת השמות החיה מהאילוץ ומוסיף אחד**, במקום להעתיק רשימה: כך נכשל ‏214 ב-21.09
כשהאילוץ החי כבר נשא שם שהקובץ לא ידע עליו. לכן ‏233 מוחל בכל סדר מול
‏214/227/229 ואינו יכול להשמיט שם. עד שיוחל, הקרון רושם שהכין דוח ולא יכול
לשלוח עליו ‏(‏23514, מטופל).

## 2026-09-21: 232 WRITTEN, not applied - שירות עצמי לספק: שני שדות פרופיל, הצעות מחיר, ותמונות בדלי ממתין

‏`232_supplier_self_service.sql`. סעיף ‏54. שתי עמודות על ‏`suppliers`
‏(‏`about_he`, ‏`opening_hours`, עד ‏300 תווים), הרחבת ה-CHECK של
‏`supplier_contact_requests` כך ששתיהן נכנסות לאותו תור בקשות ‏(מבוצע רק אם
‏225 כבר חיה, אחרת מדולג בשקט), שתי טבלאות חדשות עם ‏RLS, ודלי אחסון **פרטי**
‏`supplier-pending` בלי אף מדיניות לתפקידי לקוח.

- **‏`supplier_price_proposals`**: ספק מציע מחיר מדבקה חדש **למוצר שלו**. אגורות
  שלמות, ‏CHECK על הטווח, ‏unique על הצעה פתוחה למוצר. מדיניות ה-INSERT בודקת
  גם ש-`products.supplier_id` הוא הספק המגיש: בלי זה בעל חנות ‏A היה מגיש מחיר
  למוצר של ‏B והאדמין היה רואה שורה סבירה. האישור באדמין עובר דרך
  ‏`buildProductMoneyWrite`, אותו מודול שהטופס והקופה משתמשים בו.
- **‏`supplier_image_submissions`**: העלאה של תמונת מוצר או לוגו. **אין מדיניות
  ‏INSERT ללקוח בכוונה**: השרת מעלה את האובייקט ל-`supplier-pending` וכותב את
  השורה עם ‏service role, כדי ששורה לא תוכל לנקוב באובייקט שמעולם לא הועלה.
  אישור מעתיק לדלי הציבורי הקיים ‏(‏`product-images` / ‏`vendor-logos`) ומצרף
  ל-`products.images` או מעדכן ‏`suppliers.logo_url`.

**הוכח ‏21.09 בטרנזקציה מגולגלת על פרודקשן:** שתי הטבלאות, ‏5 מדיניות והדלי
נוצרים, ואחרי ‏ROLLBACK אף אחד מהם לא נשאר. **סדר:** אחרי ‏225 כדי ששני שדות
הפרופיל יהיו ניתנים לבקשה; בלי תלות בכל קובץ אחר. ‏`docs/SUPPLIER-SELF-SERVICE.md`.

## 2026-09-21: הדרייב-ראן של סעיף 43. תשעה קבצים כאן כבר חיים בפרודקשן, ואף אחד לא רשם

**נמדד ב-21.09, לקריאה בלבד, דרך ‏`api.supabase.com/v1/projects/<ref>/database/query`:**
לכל קובץ כאן נשלפו האובייקטים שהוא יוצר (טבלאות, פונקציות, טיפוסים, אינדקסים,
עמודות, מדיניות) ונבדק אם הם קיימים. **קיימים במלואם: ‏188, ‏189, ‏190, ‏191,
‏194, ‏196, ‏197, ‏201, ‏218.** ‏`26bcfc695` אומר ש-192 עד ‏201 הוחלו ב-09.09
ואף שורה כאן לא עודכנה. ‏218 קיים ‏(‏`enforce_profile_privilege_columns`) בלי
שום קומיט שאומר זאת. **חלקית: ‏203** (‏`support_tickets` ו-`support_ticket_messages`
קיימות עם המדיניות, ‏`support_tickets_open_queue_idx` ואינדקסים נוספים חסרים).
**‏223 ו-224 לא הוחלו**, אף ש-`0f485ab5e` ו-`d5326d44d` אומרים שכן: ל-`notifications`
אין ‏`outbox_id`, ול-`authenticated` אין ‏EXECUTE על ‏`fn_record_recent_search`.
**כל השאר לא הוחלו**, כולל ‏231.

**הקבצים החיים אינם נמחקים מכאן בסשן הזה:** העברה ל-`supabase/migrations/`
נוגעת בפנקס שסשנים אחרים כותבים בו, והמדידה חשובה יותר מהסידור. מי שמחיל
"את כל התור לפי הסדר" חייב לדעת שתשעה מהם כבר בפנים, ושרובם ‏idempotent ולכן
עוברים בשקט, **ו-190 אינו:** הוא מפיל את הגרסה הדו-ארגומנטית של
‏`fn_due_abandoned_carts` ויוצר תלת-ארגומנטית, והתלת-ארגומנטית כבר שם.

**הדרייב-ראן עצמו:** כל קובץ, בסדר מספרי, בטרנזקציה אחת מצטברת שהתגלגלה
אחורה בסופה. ‏`BEGIN;`/`COMMIT;` שבתוך הקבצים הוסרו לצורך העטיפה. ‏36 עברו,
‏3 נכשלו, ‏2 דולגו ‏(162 על ‏vault, ‏184 על חלון תחזוקה). **‏214 נכשל על
השמירה של עצמו**, כי האילוץ החי נושא ‏`gift_card_issued` שהקובץ לא ציין,
**ותוקן כאן** (הרשימה והספירה, ‏18 במקום ‏17). **‏217 נכשל כי ‏218 חייב לקדום
לו**, בדיוק כפי שהשמירה שלו אומרת, והוכח ‏218←217←214 בטרנזקציה מגולגלת.
**‏190 נכשל כי הוא כבר הוחל.** אחרי התיקון: ‏38 הקבצים הישימים עוברים בסדר
מספרי עם ‏218 לפני ‏217. ‏`docs/GO-LIVE-DRY-RUN.md`.

## 2026-09-20: 231 WRITTEN, not applied - ייצוא ומחיקה של נתוני משתמש, עם תקופת חסד

‏`231_pending_exports_deletions.sql`. שתי טבלאות (‏`pending_exports`,
‏`pending_deletions`), שני ‏enums, מדיניות ‏RLS לכל אחת, ורישום ל-audit. לא
נמחק שום אובייקט קיים.

**המספר היה ‏230 והוא היה תפוס.** הקובץ נכתב כ-`230-pending-exports-deletions.sql`,
עם מקף במקום קו תחתון, בזמן ש-`230_revoke_surplus_client_dml.sql` כבר החזיק את
המספר. זו טעות אחת שנושאת שתי תקלות: למספר כפול אין סדר החלה מוגדר, והביטוי
‏`\d{3}[a-z]?_` שקורא את הפנקס הזה דורש קו תחתון מיד אחרי הספרות ולכן לא היה
מתאים למקף בכלל. כלומר הקובץ לא היה נספר כאן גם אם מישהו היה כותב לו שורה.
שונה ל-231 ב-20.09.

**המחיקה בפועל אינה בבקשה אלא בסריקה.** ‏`/api/account/delete` רק רושם שורה
ב-`pending_deletions` ומתארך אותה ב-30 יום, ו-`/api/cron/anonymize-user-data`
הוא הקורא היחיד שהופך אותה לסופית. הוא נרשם ב-`scripts/cron-jobs.json` באותה
אצווה.
...
ולא נמחק שום אובייקט.

**‏`docs/SECURITY-POSTURE.md` §4 קורא לזה "הפער המבני היחיד" ומודד ‏56 יחסים.
נמדד מול פרודקשן ‏10.09: ‏74 ל-INSERT, ‏72 ל-UPDATE ו-72 ל-DELETE.** הוא גדל
לבד, וזה הממצא ולא המספר: הרשאות ברירת המחדל של ‏Supabase נותנות לתפקידי הלקוח
הכל על כל טבלה חדשה, ולכן כל מיגרציה שיוצרת טבלה מרחיבה את זה אלא אם אמרה
אחרת - ואף אחד לא ספר.

**‏114 מההרשאות בלתי שמישות, וזה מה שהופך אותן לניתנות לתיקון היום.** שלוש על
כל אחד מ-38 יחסים - ‏33 טבלאות, בהן ‏`payments`, ‏`vouchers`, ‏`refunds` וכל
טבלאות הארנק, ועוד ‏5 ‏views - שאין להם אף מדיניות ‏PERMISSIVE שמתירה כתיבה
לתפקיד לקוח. ‏Postgres דוחה כתיבה כזאת **לפני** שהוא בודק הרשאה, ולכן כל כתיבה
כזאת נכשלת כבר עכשיו וה-REVOKE אינו יכול לשבור מסלול שעובד.

**מה זה כן קונה:** היום מדיניות אחת שנכתבה ‏permissive בטעות על טבלת כסף היא
חולשה חיה, כי ההרשאה מתחתיה כבר שם ומחכה. אחרי הקובץ אותה טעות אינרטית עד
שמישהו גם יכתוב ‏GRANT.

**חמשת ה-views נבדקו בנפרד**, כי ל-view אין ‏RLS משלו: כל שימוש בכל החמישה
ב-`src/` וב-`apps/` הוא ‏`.select()` (נקרא ‏10.09).

**הוכח מול פרודקשן בתוך ‏`DO` שהתגלגל אחורה** ולא הוחל: כל ‏114 בוטלו, אומת
שלא נשארה אף אחת, ואומת ששני הדברים שאסור להם לזוז שרדו -

    privileges before=114, remaining after revoke=none,
    carts anon INSERT=t, notifications.read_at UPDATE=t

עגלת אורח (‏`anon` על ‏`carts`) והרשאת העמודה שדרכה הפעמון מסמן "נקרא". הקובץ
בודק את שניהם שוב אחרי ה-REVOKE של עצמו ומתפוצץ אם אחד מהם נעלם.

**הבלוק הראשון מסרב לרוץ** אם יעד קיבל מדיניות כתיבה ללקוח מאז ‏10.09, או אם
יעד קיבל הרשאה ברמת עמודה. שניהם אומרים שפיצ'ר תלוי עכשיו בהרשאה שעומדת
להתבטל. **ההבחנה הזאת דרשה ‏`pg_attribute.attacl`**: ‏
‏`information_schema.role_column_grants` מקרין הרשאת טבלה על כל עמודה ואינו
מבדיל בין השתיים - בפרודקשן יש בדיוק ארבע הרשאות עמודה אמיתיות.

**הקובץ לא משנה את ‏default privileges בכוונה.** זה היה מטפל בגידול ולא במצבור,
והיה שובר בשקט את המיגרציה הבאה שיוצרת טבלה שלקוח כן צריך לכתוב אליה - הכותב
היה מקבל דחייה בלי רמז שברירת המחדל זזה. במקום זה יש שער:
‏`client_dml_grants` ב-`supabase/rls-manifest.json` מחזיק את הרשימה שנמדדה,
ו-`src/lib/auth/rls-manifest.test.ts` מאדים כשהפנקס והקובץ מפסיקים למנות את
אותם יחסים.
...
**מה שקיים היום מתעד את הקריאה, לא את העבודה.** ‏`scripts/run-cron-jobs.sh`
סופר כל ‏2xx כהצלחה, ולכן משימה שהחזירה ‏200 בלי לעשות דבר (כי תלות לא מוגדרת)
נראית זהה למשימה שרוקנה תור. שלושה חורים, כולם נמדדו ‏10.09 ולא שוערו:

- **התיעוד נעלם.** יומני ‏Actions פגים, וסדרת cron נעצרת אחרי ‏60 יום בלי
  commit. "האם ההתחשבנות רצה ברבעון שעבר" אינה שאלה ליומן ‏CI.
- **קריאה שנהרגה אינה נראית בכלל.** פונקציה שחורגת מהזמן אינה מחזירה סטטוס.
  ‏curl מדווח כישלון, ואף אחד לא אומר איזו מ-17 המשימות מתה ועד היכן הגיעה.
- **שורת ה-`running` היא התשובה לזה**, וזו הסיבה שזה ‏INSERT ואז ‏UPDATE ולא
  ‏INSERT אחד בסוף. שורה שנשארה ‏`running` שעה אחרי שהתחילה היא העקבה היחידה
  שקריאה הרוגה משאירה, ו-`job_runs_health()` סופרת אותה ככישלון במכוון.

‏`job_runs_health()` סופרת אחורה מהריצה האחרונה ועוצרת בהצלחה הראשונה. ספירה
של "כשלים בחלון האחרון" הייתה חוצה התאוששות וממשיכה להתריע אחרי שהמשימה חזרה
לעבוד.

‏`prune_job_runs(30)` נקראת מ-`/api/cron/retention`. ‏17 משימות, שלוש מהן כל
חמש דקות, הן כ-5,000 שורות ביום; טלמטריה בלי גבול הופכת לתקלה שהיא נועדה לדווח
עליה. **היא לעולם לא מוחקת שורת ‏`running`**, בכל גיל: ראיה שפגה בטיימר היא
בדיוק הראיה שנעלמת כשמישהו סוף סוף מסתכל.

**הטבלה נעולה** באותה צורה שמיגרציה ‏172 התקינה על שמונה הטבלאות האחרות:
מדיניות ‏RESTRICTIVE שאי אפשר להצביע נגדה, **ובנוסף** ‏`REVOKE` של ההרשאה
עצמה, כי מדיניות ‏SELECT מתירנית שמישהו יוסיף מאוחר יותר הייתה נותנת לאותו
תפקיד גם ‏INSERT, ‏UPDATE ו-DELETE.

**כל צרכן מתדרדר ואינו קורס.** ‏`withJobRun` בולע ‏PGRST205 ולא רושם כלום, מסך
האדמין אומר במילים שהטבלה חסרה במקום לצייר ‏17 שורות ריקות, וההתראה על שני
כשלים ברצף נשארת ‏null. הקובץ הזה כשאינו מוחל עולה נראות ולא שובר דבר.
