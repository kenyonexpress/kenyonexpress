...
**18 מתוך 18 סגורים. אין פריט פתוח.** ‏[1]-[12] אומתו מחדש בסשן נקי
ב-01.08 בלי לקבל את התיעוד כנתון: `tsc` נקי, **1235/1235** Vitest, build
נקי, **61/61** Playwright מול build טרי על פורט פנוי. אפס רגרסיה מול
`e067e0b`.

מ-[15] והלאה כל שלב נפתח ממה שקדם לו השאיר פתוח, וכל אחד מהם מצא שהשלב
הקודם צדק בכיוון וטעה בהיקף. **המועמד הבא באותו קו נמדד ולא נוחש**:
‏`uses-responsive-images` בדף הבית מדווח **400KiB, וכולם 32 כרטיסי הדילים**
שמוגשים ב-w=384 כדי לצייר 157px. מדרגת 288 ב-`images.imageSizes` חותכת
כרבע מזה, והיא שינוי קונפיגורציה שנוגע לכל תמונה באתר ולכן דורש מדידה
משלו. אחריו: אלמנט ה-LCP של הבית הוא עדיין באנר הקוקיז, ‏78 בנייד.
