...
|---|---|---|
| 033 | `supabase/migrations/058_ledger_core.sql` | טבלאות ledger, אילוצים, אינדקסים, sum-zero, immutability |
| 034 | `supabase/migrations/059_money_integer_units.sql` | כסף numeric לאגורות integer, אחוזים לנקודות בסיס |
| 035 | `supabase/migrations/060_idempotency_keys.sql` | טבלת idempotency_keys |
| 036 | `supabase/migrations/061_coupon_single_use.sql` | הקשחת מימוש קופון חד-פעמי |
| 037 | `supabase/migrations/062_settlement_batches.sql` | settlement_batches + settlement_items פר ספק |
| 038 | `supabase/migrations/063_reconciliation.sql` | reconciliation_runs + reconciliation_discrepancies |
| 039 | `supabase/migrations/064_money_rls.sql` | RLS לכל הטבלאות החדשות |
...
- כל אחוז הוא `integer` בנקודות בסיס (basis points): 10% = 1000 bp, 100% = 10000 bp.
  ההמרה מאחוז עשרוני היא הכפלה ב-100 בדיוק כמו כסף, ולכן מיגרציה 059 משתמשת באותו helper.
...
- שלושת החשבונות הגלובליים נזרעים במיגרציה 058. חשבונות ספק ולקוח נוצרים בעצלנות דרך
...
- RLS (מיגרציה 064) מוסיף שכבה שנייה: אין שום policy של INSERT/UPDATE/DELETE לאף רול
...
- פונקציות SQL קיימות שקוראות עמודות בשמן הישן (למשל `product_platform_percent`,
  פונקציות ה-settlement של 027) נשברות אחרי ה-rename; מיגרציה 059 מגדירה מחדש את
