...
בשימוש הוא 049. לכן הקבצים ממוספרים 050 עד 056, והמיפוי מתועד גם בכותרת של כל קובץ:

| מספר במפרט | קובץ בפועל | תוכן |
|---|---|---|
| 033 | `supabase/migrations/050_ledger_core.sql` | טבלאות ledger, אילוצים, אינדקסים, sum-zero, immutability |
| 034 | `supabase/migrations/051_money_integer_units.sql` | כסף numeric לאגורות integer, אחוזים לנקודות בסיס |
| 035 | `supabase/migrations/052_idempotency_keys.sql` | טבלת idempotency_keys |
| 036 | `supabase/migrations/053_coupon_single_use.sql` | הקשחת מימוש קופון חד-פעמי |
| 037 | `supabase/migrations/054_settlement_batches.sql` | settlement_batches + settlement_items פר ספק |
| 038 | `supabase/migrations/055_reconciliation.sql` | reconciliation_runs + reconciliation_discrepancies |
| 039 | `supabase/migrations/056_money_rls.sql` | RLS לכל הטבלאות החדשות |
...
- כל אחוז הוא `integer` בנקודות בסיס (basis points): 10% = 1000 bp, 100% = 10000 bp.
  ההמרה מאחוז עשרוני היא הכפלה ב-100 בדיוק כמו כסף, ולכן מיגרציה 051 משתמשת באותו helper.
- עיגול: `round()` של Postgres (half up על numeric חיובי) בכל הכפלה של סכום באחוז:
  `round(amount_agorot * bp / 10000.0)::integer`.

אסטרטגיית ההמרה במיגרציה 051 (add / backfill / verify / rename):
...
`platform_price` ו-`discount_percentage`, לא ניתנות להמרה במקום ולכן נמחקות ונוצרות
מחדש ביחידות החדשות: `platform_price_agorot` ו-`discount_bp`.
...
- שלושת החשבונות הגלובליים נזרעים במיגרציה 050. חשבונות ספק ולקוח נוצרים בעצלנות דרך
...
- RLS (מיגרציה 056) מוסיף שכבה שנייה: אין שום policy של INSERT/UPDATE/DELETE לאף רול
...
- פונקציות SQL קיימות שקוראות עמודות בשמן הישן (למשל `product_platform_percent`,
  פונקציות ה-settlement של 027) נשברות אחרי ה-rename; מיגרציה 051 מגדירה מחדש את
