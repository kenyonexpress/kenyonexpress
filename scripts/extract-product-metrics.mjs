...
const probe = async (url, wait) => {
  ...
  const data = await p.evaluate(() => {
    ...
    const out = {}
    out.breadcrumb = info(
      q('.woocommerce-breadcrumb') ||
        q('nav[aria-label*="breadcrumb" i]') ||
        q('[class*="breadcrumb" i]'),
    )
    out.title = info(q('h1'))
    out.gallery = info(
      q('.woocommerce-product-gallery') || q('[class*="gallery" i]') || q('main img'),
    )
    out.priceWrap = info(q('.summary .price') || q('[class*="price" i]'))
    out.priceCurrent = info(
      q('.summary .price ins .amount') ||
        q('.summary .price > .amount') ||
        byText('[class*="price" i] *', '99'),
    )
    ...
    out.addToCart = info(
      q('.single_add_to_cart_button') ||
        byText('button', 'הוספה לסל') ||
        byText('button', 'הוסף לסל'),
    )
    out.buyNow = info(byText('button, a', 'קנה עכשיו') || byText('button, a', 'קנייה מהירה'))
    out.related = info(
      q('.related') ||
        byText('h2, section h2, [class*="related" i] h2', 'מומלצים') ||
        byText('h2', 'מוצרים נוספים'),
    )
