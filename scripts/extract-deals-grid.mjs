...
const out = await page.evaluate(() => {
  ...
  const items = [...grid.querySelectorAll('.jet-listing-grid__item')].map((el) => {
    ...
    const badge = [...el.querySelectorAll('*')].map((n) => n.childNodes.length === 1
      && n.textContent?.trim().match(/^-\d+%$/) ? n.textContent.trim() : null).find(Boolean)
    const prices = [...el.querySelectorAll('.woocommerce-Price-amount, .price')]
    ...
    return {
      ...
      imgBox: imgRect ? { x: Math.round(imgRect.x), y: Math.round(imgRect.y + scrollY), w: Math.round(imgRect.width), h: Math.round(imgRect.height) } : null,
      badge,
      prices,
      priceText,
