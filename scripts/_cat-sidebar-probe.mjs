...
for (const url of urls) {
  ...
  const out = await p.evaluate(() => {
    const r = (el) => {
      ...
      return { cls: el.className.toString().slice(0, 60), top: +(b.top + scrollY).toFixed(0), left: +b.left.toFixed(0), w: +b.width.toFixed(0), h: +b.height.toFixed(0) }
    }
    const main = document.querySelector('main.site-main, .content-area')
    const shopSidebar = document.querySelector('#sidebar-shop, .sidebar-shop, aside#secondary, .widget-area')
    ...
    return {
      ...
      asidesAboveFooter: [...document.querySelectorAll('aside')]
        .map((el) => r(el))
        .filter((x) => x && x.top < (document.querySelector('footer')?.getBoundingClientRect().top ?? 1e9) + scrollY),
