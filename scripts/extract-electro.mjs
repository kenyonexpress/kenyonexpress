...
const tokens = await page.evaluate(() => {
  ...
  result.header = {
    ...
    search_input: {
      rect: rect(searchInput),
      style: styleOf(searchInput, ['height', 'font-size', 'border-top-width', 'background-color']),
    },
  }

  // ===================== HERO SLIDER =====================
  const revContainer = pick([
    'rs-module-wrap',
    '.rev_slider_wrapper',
  ...
  const sideBanner = pick([
    '.slider-das-block a',
    '.slider-das-block img',
    '.slider-das-block > div',
  ])

  result.hero = {
    container: {
      rect: rect(revContainer),
      style: styleOf(revContainer, ['height', 'background-color']),
    },
    ...
    dots: {
      rect: rect(dots),
      style: styleOf(dots, ['bottom', 'top', 'left', 'right', 'text-align']),
    },
    dot: {
      rect: rect(dot),
      style: styleOf(dot, [
  ...
  const cardInner = card
    ? card.querySelector('.product-outer, .product-inner, .content-product')
    : null
  const cardTitle = card
    ? card.querySelector('.woocommerce-loop-product__title, .product-title, h2, h3, .product_title')
    : null
  ...
  const sectionHeadings = Array.from(
    document.querySelectorAll(
      'h2.h1, .section-title-normal h2, .products-carousel-header h2, .vc_custom_heading',
    ),
  )
    .filter((el) => {
      const r = el.getBoundingClientRect()
  ...
  const footerWidgets = pick([
    '.footer-widgets',
    '#colophon .footer-widgets',
    '.footer-widget-area',
  ])
  const footerBottom = pick([
    '.copyright-bar',
    '#colophon .footer-bottom-widgets',
    '.footer-bottom',
    '.site-info',
  ])
