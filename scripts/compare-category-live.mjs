...
// Writes refs/live-category-cmp.png + refs/mine-category-cmp.png and prints band diff.
...
const LIVE_URL = `https://kenyonexpress.co.il/product-category/${encodeURIComponent('מסעדות-ובתי-קפה')}/`
const MINE_URL = 'http://localhost:3000/category/restaurants-cafes'
...
await live.screenshot({ path: 'refs/live-category-cmp.png', fullPage: true })
console.log('live-category-cmp.png written')

const mine = await ctx.newPage()
await mine.goto(MINE_URL, { waitUntil: 'networkidle', timeout: 60000 })
await mine.waitForTimeout(2000)
await mine.screenshot({ path: 'refs/mine-category-cmp.png', fullPage: true })
console.log('mine-category-cmp.png written')
...
const report = await page.evaluate(
  ...
  {
    liveUrl: toDataUrl('refs/live-category-cmp.png'),
    mineUrl: toDataUrl('refs/mine-category-cmp.png'),
  },
)
await b2.close()
