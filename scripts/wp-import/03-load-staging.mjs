...
const TABLES = [
  {
    ...
    entity: 'category',
    key: 'wp_term_id',
    columns: ['wp_term_id', 'batch_id', 'slug_raw', 'slug_decoded', 'name_he', 'description',
      'parent_wp_id', 'product_count', 'thumbnail_wp_id', 'permalink', 'raw'],
  },
  {
    name: 'products',
    file: 'products',
    entity: 'product',
    key: 'wp_post_id',
    columns: ['wp_post_id', 'batch_id', 'post_type', 'wp_parent_id', 'slug_raw', 'slug_decoded',
      'permalink', 'title_he', 'description_html', 'excerpt_html', 'status_raw', 'sku',
      'regular_price', 'sale_price', 'price', 'currency', 'manage_stock', 'stock_status_raw',
      'stock_quantity', 'is_virtual', 'product_type_raw', 'total_sales', 'attributes',
      'category_wp_ids', 'tag_names', 'featured_image_wp_id', 'gallery_wp_ids',
      'seo_title_raw', 'seo_description_raw', 'created_at_wp', 'modified_at_wp',
      'proposed_slug', 'target_type', 'raw_meta', 'raw_post'],
  },
  {
    name: 'media',
    file: 'media',
    ...
    columns: ['wp_attachment_id', 'batch_id', 'wp_parent_post_id', 'source_url', 'file_name',
      'mime_type', 'alt_text', 'title', 'width', 'height', 'byte_size', 'sha256', 'status',
      'bucket', 'created_at_wp', 'raw'],
  },
  {
    name: 'customers',
    file: 'customers',
    entity: 'customer',
    key: 'wp_user_id',
    columns: ['wp_user_id', 'batch_id', 'email_raw', 'email_normalized', 'display_name',
      'first_name', 'last_name', 'phone_raw', 'phone_normalized', 'registered_at',
      'last_active_at', 'is_paying_customer', 'orders_count', 'total_spent', 'billing',
      'shipping', 'newsletter_optin_raw', 'raw_meta'],
  },
  {
    name: 'orders',
    file: 'orders',
    entity: 'order',
    key: 'wp_order_id',
    columns: ['wp_order_id', 'batch_id', 'storage_source', 'order_number', 'status_raw',
      'currency', 'customer_wp_id', 'billing_email', 'billing_phone', 'billing', 'shipping',
      'subtotal', 'discount_total', 'shipping_total', 'tax_total', 'total', 'refunded_total',
      'payment_method', 'payment_method_title', 'transaction_id', 'customer_note',
      'created_at_wp', 'paid_at_wp', 'completed_at_wp', 'raw'],
  },
  {
    ...
    columns: ['wp_order_item_id', 'wp_order_id', 'batch_id', 'item_type', 'product_wp_id',
      'variation_wp_id', 'item_name', 'quantity', 'line_subtotal', 'line_total', 'line_tax', 'meta'],
  },
  {
    name: 'coupons',
    file: 'coupons',
    entity: 'coupon_code',
    key: 'wp_post_id',
    columns: ['wp_post_id', 'batch_id', 'code', 'discount_type', 'amount', 'usage_limit',
      'usage_count', 'expires_at_wp', 'status_raw', 'raw_meta'],
  },
  {
    name: 'url_inventory',
    file: 'url_inventory',
    entity: 'redirect',
    key: 'old_path',
    columns: ['old_path', 'batch_id', 'sources', 'entity', 'entity_wp_id', 'mapped_new_path',
      'direct_match', 'gone_410', 'mapping_rule'],
  },
]
...
export async function loadStaging(run) {
  ...
  for (const table of tables) {
    ...
    try {
      ...
      info(`  ${table.name.padEnd(14)} ${String(rows.length).padStart(6)} rows  (+${inserted} new, ~${updated} existing)`)
    } catch (err) {
  ...
  if (issues.length > 0 && db && !DRY_RUN) {
    const { error } = await db
      .schema('wp_import')
      .from('issues')
      .upsert(issues.map((i) => ({ ...i, batch_id: batchId })), { onConflict: 'entity,wp_id,code' })
