...
for (const p of missing) {
  ...
  // import; both are a decision, so the row waits for the migration and a human.
  // Zero rows hit this in the 2026-07-29 export; see `mapType` in lib/clean.mjs.
...
lines.push(
  `-- ${missing.length} not in production; ${skippedNoPrice} skipped for price, ` +
    `${skippedNoImage} for images, ${skippedRecurring} for recurring.`,
)
...
process.stderr.write(
  `missing=${missing.length} excluded_scaffolding=${excluded.length} ` +
    ...
    `skipped_recurring=${skippedRecurring} slug_title_mismatch=${mismatched} ` +
    `emitted=${missing.length - skippedNoPrice - skippedNoImage - skippedRecurring}\n`,
)
