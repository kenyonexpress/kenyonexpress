...
| Contract (this file) | Live (read from `kenyonexpress` source 2026-09-09; production counts 2026-09-01 unless a later date is named) |
|---|---|
| Next.js 15 + Cloudflare Workers + Hono | Next.js 16.3.3 App Router on Vercel (`kenyonexpress` checkout). This worktree still pins 16.2.12. No Hono process. No `wrangler.toml`. Cloudflare holds DNS and R2. |
...
This matrix is the **contract** for the tables the nine surfaces touch. The last MCP pull of the live inventory is 2026-09-01 in `docs/ARCHITECTURE-OVERVIEW.md`: 61 tables, 133 policies, 0 RLS off. Generated types in kenyonexpress `src/types/database.ts` now list **81** public tables (this pass, 2026-09-09), so 61 is a dated pull, not a current type dump. `docs/DB-SECURITY-MODEL.md` §0 still says 53 tables; that is the older narrower pull, not the inventory. Where they disagree on a live grant, prefer the policy text in the security model over a count. Where they disagree on what the policy must be, this file wins.
...
Live also meters money-adjacent paths the contract table does not name. The one that is already minute-scale: `begin_checkout` 10 per 60 seconds (LowProfile deal creation). OTP is per IP **and** per destination number. MFA verify is 10 per 15 minutes per user. The rest of `RATE_LIMIT_POLICIES` (**40** named rows, counted in kenyonexpress `src/lib/rate-limit/policies.ts` this pass) is the inventory; a new call site that is not in that table is a hole the unit test is written to catch.
...
| 2026-09-09 | Fifth source pass: live Next is 16.3.3 in the kenyonexpress checkout (this worktree still 16.2.12). Rate-limit table is exactly 40 rows. Generated types list 81 public tables against the 2026-09-01 MCP count of 61. Re-measured `whatsapp` 404 and `/api/health` 200 on `https://kenyonexpress.vercel.app`. |
